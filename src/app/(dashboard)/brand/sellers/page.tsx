...
export default async function BrandSellersPage() {
  ...
  // 병합된 상담사 프로필에는 Decimal 필드(commissionRate 등)가 포함돼 있어
    ...
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">브랜드 상담상품을 판매하는 상담사 {sellers.length}명</p>
        </div>
      </div>
      ...
      {sellers.length === 0 ? (
        <div className="text-center py-20 text-gray-400 bg-white rounded-xl border border-gray-100">
          ...
          <p className="text-sm">아직 상담상품을 판매하는 상담사가 없습니다.</p>
          <p className="text-xs text-gray-300 mt-1">상담사가 브랜드 상담상품을 점집에 추가하면 자동으로 표시됩니다.</p>
        </div>
      ) : (
