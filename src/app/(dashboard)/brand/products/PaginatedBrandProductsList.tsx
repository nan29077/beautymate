                ...
                <div className="flex items-center gap-3">
                  <Link href={`/products/${product.id}`} className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                        ...
                        {showSellers ? (
                          <span className="text-[10px] text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded font-medium">
                            판매중 {activeSellers.length}개 점집
                          </span>
                        ) : (
                          <span className="text-[10px] text-gray-400">
                            상담사 {product._count.sellerProducts} · 공구 {product._count.campaigns} · 후기 {product._count.reviews}
                          </span>
                        )}
                      </div>
                      {showSellers && (
                    ...
                    <div className="text-right flex-shrink-0">
                      ...
                      >
                        {!product.isActive ? "숨김" : product.isApproved ? "상담사 공개중" : "검수중(비공개)"}
                      </span>
                    </div>
                  </Link>
                  <div className="flex items-center gap-1 flex-shrink-0">
                    ...
                    />
                    <ProductItemActions productId={product.id} mode="brand" isActive={product.isActive} />
                  </div>
                </div>
                ...
                {filterTab === "all" && product.sellerProducts.length > 0 && (
                  <div className="mt-2 ml-[52px]">
                    <div className="flex items-center gap-1 mb-1.5">
                      ...
                      <span className="text-[10px] text-gray-400 font-medium">판매 상담사 ({product.sellerProducts.length})</span>
                    </div>
