...
import { Plus, BookOpen, BarChart3, MessageCircle } from 'lucide-react';
...
export default async function BrandProductsPage() {
  ...
  // 판매 중인 상담상품 — 승인되어 상담사가 현재 판매 중(승인+활성)인 본 브랜드 상담상품
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900">상담상품 관리</h1>
          <p className="text-sm text-gray-500">{brand.products.length}개의 상담상품</p>
        </div>
      ...
      {pendingApplications.length > 0 && (
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-bold text-gray-700">상담사 신청 대기 ({pendingApplications.length})</h2>
          </div>
          <PendingSellerApproval
            applications={pendingApplications.map((app) => ({
              ...
              sellerPrice: null, // 브랜드사에게는 상담사 판매가 비공개
      ...
      {soldProducts.length > 0 && (
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-bold text-gray-700">판매 중인 상담상품 ({soldProducts.length})</h2>
          </div>
      ...
      {brand.products.length === 0 ? (
        <div className="text-center py-20 text-gray-400 bg-white rounded-xl border border-gray-100">
          ...
          <p className="text-xs text-gray-300 mt-1">상담상품 등록 버튼을 눌러 첫 상담상품을 등록하세요.</p>
        </div>
      ) : (
