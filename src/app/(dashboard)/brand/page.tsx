    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <Link href="/brand/products" className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 text-xs font-medium text-white bg-gray-900 rounded-lg hover:bg-gray-800 transition-colors flex-shrink-0">
          ...
          <span className="hidden sm:inline">상담상품 등록</span><span className="sm:hidden">등록</span>
        </Link>
      </div>
      ...
      {!brand.isApproved && (
        <div className="p-3.5 bg-amber-50 border border-amber-100 rounded-xl text-xs text-amber-700 flex items-center gap-2">
          ...
          브랜드 승인 대기 중입니다. 관리자 승인 후 상담상품이 노출됩니다.
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3">
        ...
        <div className="bg-gray-900 rounded-xl p-3 sm:p-4 text-white">
          <p className="text-gray-300 text-[9px] sm:text-[10px] font-medium mb-1">연결 상담사</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
        {[
          { label: "전체 상담상품", value: totalProducts, icon: "Package", color: "text-blue-500" },
          { label: "활성 상담상품", value: activeProducts, icon: "Store", color: "text-green-500" },
          { label: "진행중 캠페인", value: activeCampaigns, icon: "Event", color: "text-purple-500" },
          { label: "총 후기", value: totalReviews, icon: "Star", color: "text-amber-500" },
        ].map((kpi) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-50">
            <h2 className="text-sm font-bold text-gray-900">내 상담상품</h2>
          ...
          {brand.products.length > 0 ? (
                ...
                <div key={product.id} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50/50 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 text-[11px] text-gray-400">
                      <span>상담사 {product._count.sellerProducts}</span>
                      <span>·</span>
                      <span>후기 {product._count.reviews}</span>
                      <span>·</span>
            ...
            <div className="text-center py-10">
              ...
              <p className="text-xs text-gray-400 mb-1">등록된 상담상품이 없습니다.</p>
              <Link href="/brand/products" className="text-xs text-blue-500 hover:underline">첫 상담상품 등록하기</Link>
            </div>
          )}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
          {[
            { href: "/brand/products", label: "상담상품 관리", icon: "Package", desc: "상담상품 등록 & 수정" },
            { href: "/brand/campaigns", label: "캠페인", icon: "Event", desc: "공구 현황" },
            { href: "/brand/sellers", label: "상담사", icon: "Store", desc: "연결 상담사" },
            { href: "/brand/orders", label: "예약", icon: "OrderManagement", desc: "예약 확인" },
