...
import { BarChart3, Star, TrendingUp, BookOpen } from "lucide-react";
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
        {[
          { label: "전체 상담상품", value: totalProducts, icon: Star, color: "bg-blue-500" },
          { label: "총 판매량", value: totalSold.toLocaleString(), icon: BookOpen, color: "bg-green-500" },
          ...
          { label: "총 후기", value: totalReviews, icon: Star, color: "bg-yellow-500" },
          { label: "입점 상담사", value: totalSellers, icon: Star, color: "bg-pink-500" },
        ].map((kpi) => (
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-5">
        <h3 className="text-sm font-bold text-gray-900 mb-4">인기 상담상품 TOP 5</h3>
        {topProducts.length === 0 ? (
          <p className="text-sm text-gray-400">상담상품 데이터가 없습니다.</p>
        ) : (
