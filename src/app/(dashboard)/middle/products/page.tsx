...
export default async function MiddleProductsPage() {
  ...
  const [products, brands, pendingApplications, soldProducts] = await Promise.all([
    ...
    // 상담사 신청 승인 대기 목록 — 중간관리자 관할 상담상품(자체 등록 + 소속 브랜드 상담상품)
    ...
    // 판매 중인 상담상품 — 중간관리자 관할 상담상품 중 상담사가 현재 판매 중(승인+활성)
    prisma.sellerShopProduct.findMany({
      where: {
        isApproved: true,
        isActive: true,
  ...
  const initialProducts = products.map((p) => ({
    ...
    // 중간관리자 본인이 직접 등록한 상담상품 여부 (브랜드 없음) → 최고관리자 승인 대상
    isOwn: !p.brandId,
    // 판매중(승인+활성) 상담사 목록
      ...
      {serializedApplications.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-bold text-gray-700">상담사 신청 승인 대기 ({serializedApplications.length})</h2>
          </div>
      ...
      {serializedSold.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-bold text-gray-700">판매 중인 상담상품 ({serializedSold.length})</h2>
          </div>
