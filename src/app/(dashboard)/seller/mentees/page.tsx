...
import { Users, TrendingUp, Copy, Check, Calendar, Star } from "lucide-react";
...
export default function SellerMenteesPage() {
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <div>
        <h1 className="text-lg sm:text-xl font-bold text-gray-900">멘티 상담사 관리</h1>
        <p className="text-xs text-gray-400 mt-0.5">내 추천인코드로 가입한 상담사와 추천인 커미션을 관리합니다</p>
      </div>

      {/* 추천인코드/링크 카드 */}
      <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border border-blue-100 p-4 space-y-3">
        <p className="text-sm font-semibold text-gray-800 flex items-center gap-2">
          <Users size={16} className="text-blue-500" /> 내 상담사가입 추천인코드
        </p>
        {referralCode ? (
          <>
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-xs font-bold text-gray-400 uppercase tracking-wider">멘티 상담사 커미션</h2>
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-gray-100 py-12 text-center">
            ...
            <p className="text-sm text-gray-400">아직 추천한 상담사가 없습니다</p>
            <p className="text-xs text-gray-300 mt-1">추천인코드를 공유하면 가입한 상담사가 여기에 표시됩니다</p>
          </div>
        ) : (
