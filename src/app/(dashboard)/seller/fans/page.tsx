...
export default async function SellerFansPage() {
  ...
  const verifiedFollowers = seller.followers.filter((f) => approvedBuyerIds.has(f.buyerId));
  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500">총 {seller.totalFans}명의 팬</p>
      </div>
      ...
      <div className={`grid ${flags.referral ? "grid-cols-2 sm:grid-cols-4" : "grid-cols-3"} gap-2 mb-6`}>
        <div className="bg-white rounded-xl border border-gray-100 p-3 text-center">
          ...
          <p className="text-base font-bold text-gray-900">{seller.totalFans}</p>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-3 text-center">
          ...
          <p className="text-base font-bold text-gray-900">{verifiedFollowers.length}</p>
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-100">
          ...
          <p className="text-[10px] text-gray-400 mt-0.5">상담사를 PICK했거나 추천·단골로 연결된 모든 팬</p>
        </div>
        {allFans.length === 0 ? (
          <div className="text-center py-12 text-gray-400">
            <Icon name="Wishlist" size={32} strokeWidth={1.5} className="mx-auto mb-2 opacity-30" />
            <p className="text-xs">아직 픽 팬이 없습니다</p>
            <p className="text-[10px] text-gray-300 mt-0.5">팬이 상담사를 PICK하면 여기에 모여요</p>
          </div>
        ) : (
          <PaginatedFansList
