...
import { DEFAULT_PRODUCT_IMAGE, pickBeautyMateAvatar } from "@/lib/defaults";
...
const DIFFERENTIATORS = [
  { icon: Radio, color: "text-rose-500", bg: "bg-rose-50", title: "내 방송이 곧 예약창구", desc: "검색 마켓플레이스가 아니라, 뷰티 전문가가 자신의 시청자를 그대로 데려오는 구조예요." },
  { icon: Heart, color: "text-pink-500", bg: "bg-pink-50", title: "단골 중심 구조", desc: "내가 단골로 등록한 뷰티 전문가와 깊게 연결돼요. 뷰티 전문가는 팬을 고객으로 만들 수 있어요." },
  { icon: ShieldCheck, color: "text-emerald-500", bg: "bg-emerald-50", title: "안심 거래·정산", desc: "통신판매중개 보호와 투명한 정산으로 안전하게." },
  { icon: Sparkles, color: "text-brand-600", bg: "bg-brand-50", title: "AI 고객노트", desc: "요청사항을 AI가 정리해 드려요. 다시 봐도 쉽게 이해되고, 뷰티 전문가는 재방문 고객을 편하게 관리해요." },
];

// 라이브 뷰티 진행 방법 (고객 / 뷰티 전문가)
const BUYER_STEPS = ["좋아하는 뷰티 전문가를 단골로 설정해요", "방송 시작 알림을 받아요", "라이브에 입장해 소통해요", "원하는 시간에 바로 예약·결제해요"];
const SELLER_STEPS = ["뷰티 서비스와 가능한 시간을 등록해요", "평소처럼 라이브 방송을 켜요", "예약 URL·QR을 설명란에 걸어둬요", "방송이 끝나면 예약이 쌓여 있어요"];

// 메인 페이지 = "뷰티메이트 브랜드 소개 + 뷰티 전문가 진입(이름검색/단골) + 뷰티 전문가 신청 유도" 화면.
// ※ 뷰티 서비스 판매/구매 리스트(인기뷰티 서비스·공구·뷰티 서비스 그리드 등)는 두지 않는다.
//   유일한 예외: 내가 단골로 등록한 뷰티 전문가가 "지금 라이브 방송 중"인 뷰티 서비스를 뷰티 전문가별로 묶어 보여준다.
async function getHomeData(featureLive: boolean) {
  ...
  if (isBuyer) {
    ...
    liveSellers = (profile?.follows || [])
      .map((f) => f.seller)
      .map((s) => {
        // 1순위: 뷰티메이트 라이브 뷰티(실제 LIVE 방송) — 라이브 뷰티 기능 ON + 방송 진행 중
        ...
        // 2순위: 수동 "라이브 중" 스위치 ON — 실제 방송 없이도 노출 뷰티 서비스와 함께 노출
        if (s.isManualLive) {
          let ids: string[] = [];
          try {
...
const SELLER_CTA_BG_DEFAULT = "/banners/beautymate/expert-cta-v3.png";
...
export default async function HomePage({
  ...
  const [heroBannersRaw, bottomBannerRaw, activeCampaignCount] = await Promise.all([
    safeQuery("메인 히어로 배너", () => prisma.banner.findMany({
      where: { isActive: true, position: "hero" },
      orderBy: { sortOrder: "asc" },
      take: 3,
    }), []),
    safeQuery("메인 하단 배너", () => prisma.banner.findFirst({
      where: { isActive: true, position: "bottom" },
      orderBy: { sortOrder: "asc" },
    }), null),
    safeQuery("진행 중인 캠페인 수", () => prisma.groupBuyCampaign.count({ where: { status: "ACTIVE" } }), 0),
  ]);

  // 이미지 파일이 없는 배너는 서버에서 걸러 첫 렌더부터 기본(뷰티메이트) 배너가 나가게 한다
  ...
  const desktopHeroBanners = heroBanners.map((b) => ({
    id: b.id,
    title: b.title,
    subtitle: b.subtitle,
    imageUrl: b.imageUrl,
  ...
  return (
    <>
      ...
      {/* ───── 내 단골 뷰티 전문가 LIVE 방송 중 뷰티 서비스 (뷰티 전문가별 구분) ───── */}
      {liveSellers.length > 0 && (
        <section className="pt-6 pb-2">
          ...
          <div className="space-y-5">
            {liveSellers.map((s) => {
              // 프로필 클릭 → 라이브 방송으로 이동 (우선순위: 인앱 라이브 시청페이지 > 뷰티샵관리 수동 liveLink > 뷰티샵)
                ...
                <>
                  ...
                  <div className="relative w-10 h-10 flex-shrink-0">
                    <div className={`w-10 h-10 rounded-full overflow-hidden bg-gray-100 ${LIVE_RING_CLASS}`}>
                      <SafeImage src={s.shopLogo} placeholder={pickBeautyMateAvatar(s.id)} alt={s.shopName} width={40} height={40} fallbackText={s.shopName.charAt(0)} />
                    </div>
                  </div>
              ...
              const shopShortcut = (
                ...
                >
                  ...
                  <span className="text-[10px] font-semibold text-gray-600">뷰티샵 바로가기</span>
                </Link>
              );
              return (
                <div key={s.id}>
                  {/* 뷰티 전문가 헤더 — 클릭 시 라이브 방송으로 이동 (인앱 라이브 우선, 수동 연동은 liveLink) */}
                  {externalLiveUrl ? (
                  ...
                  {s.products.length > 0 ? (
                    ...
                    <div className="flex items-center gap-2.5 px-4 pb-1">
                      <p className="text-[11px] text-gray-400 flex-1">방송 뷰티 서비스 준비 중</p>
                      {shopShortcut}
                    </div>
                  )}
                </div>
      ...
      {/* 라이브 중인 단골 뷰티 전문가가 없을 때(로그인 고객에게만) 안내 */}
      {liveSellers.length === 0 && isBuyer && (
        <section className="pt-6 pb-1 px-4">
          ...
          <div className="rounded-2xl border border-gray-100 bg-gray-50/60 px-4 py-5 text-center">
            ...
            <p className="text-[12px] text-gray-500">현재 라이브 중인 단골 뷰티 전문가가 없어요</p>
            <p className="text-[11px] text-gray-400 mt-0.5">단골 뷰티 전문가가 방송을 시작하면 여기에 표시돼요</p>
          </div>
        </section>
      )}
      ...
      {/* ───── 뷰티메이트 소개: 우리는 이런 곳이에요 ───── */}
      <section className="px-5 pt-8 pb-2">
        <h2 className="text-[17px] font-extrabold text-gray-900">뷰티메이트는요</h2>
        <p className="mt-1.5 text-[13px] text-gray-500 leading-relaxed">
          스킨케어·메이크업·헤어·네일·퍼스널 컬러 전문가의 콘텐츠를 살펴보고
          나에게 맞는 서비스를 간편하게 예약하는 뷰티 플랫폼이에요.
          라이브로 전문가의 노하우를 확인하고 그 자리에서 예약까지 이어갈 수 있습니다.
        </p>
        <div className="mt-4 grid grid-cols-3 gap-2.5">
          <IntroStat icon={<Icon name="Users" size={18} className="text-brand-600" />} value="단골" label="나만의 뷰티 전문가" />
      ...
      {/* ───── 추천 뷰티 전문가 (자동 슬라이드 마퀴) ───── */}
      <section className="pt-7 pb-1">
        <div className="flex items-center gap-1.5 px-5 mb-3">
          <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
          <h2 className="text-[15px] font-extrabold text-gray-900">추천 뷰티 전문가</h2>
          <span className="text-[10px] text-gray-400">분야별 전문가 안내</span>
        </div>
        <SellerMarquee />
      </section>

      {/* ───── 뷰티 전문가가 뷰티메이트를 선택하는 이유 (수치·혜택) ───── */}
      <section className="px-5 pt-7 pb-2">
        <h2 className="text-[17px] font-extrabold text-gray-900">뷰티 전문가가 뷰티메이트를 선택하는 이유</h2>
      ...
      <section className="px-4 pt-7">
        <div className="rounded-3xl bg-gray-900 text-white p-6">
          ...
          <h2 className="text-[18px] font-extrabold">숫자로 보는 뷰티메이트</h2>
          <p className="text-[11px] text-white/60 mt-1">전문가와 고객을 잇는 새로운 뷰티 경험</p>
      ...
      <section className="pt-8 pb-1">
        <div className="px-5 mb-3">
          <h2 className="text-[17px] font-extrabold text-gray-900">뷰티메이트로 성공한 뷰티 전문가</h2>
          <p className="mt-1 text-[12px] text-gray-500">평범한 일상에서, 나만의 뷰티샵으로</p>
        </div>
          ...
          {homeStories.map((s, i) => (
            <div key={s.name} className="flex-shrink-0 w-64 rounded-2xl border border-gray-100 bg-white p-4 flex flex-col">
              ...
              <div className="flex items-center gap-2.5 mt-3 pt-3 border-t border-gray-50">
                <img src={s.avatar || "/avatars/beautymate/default.svg"} alt="" className="w-10 h-10 rounded-full bg-brand-50 ring-2 ring-brand-100 object-cover" />
        ...
        <div className="mt-4 space-y-2.5">
          <div className="rounded-2xl border border-gray-100 bg-white p-4">
            <div className="flex items-center gap-2 mb-2.5">
              ...
              <p className="text-[14px] font-bold text-gray-900">이런 뷰티 경험을 원한다면</p>
            </div>
            <ul className="space-y-1.5">
              {["믿을 수 있는 전문가를 찾고 싶어요", "콘텐츠를 먼저 보고 실력과 분위기를 확인하고 싶어요", "전화·카톡 문의 없이 바로 예약하고 싶어요"].map((t) => (
          ...
          <div className="rounded-2xl border border-brand-100 bg-brand-50/50 p-4">
            <div className="flex items-center gap-2 mb-2.5">
              ...
              <p className="text-[14px] font-bold text-gray-900">이렇게 고객을 만나고 싶다면</p>
            </div>
            <ul className="space-y-1.5">
            ...
            <Link href="/become-seller" className="mt-3 inline-flex items-center gap-1 text-[12px] font-bold text-brand-700">
              뷰티 전문가로 신청하기 <Icon name="ChevronDown" size={14} className="-rotate-90" />
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="px-5 pt-8">
        <div className="inline-flex items-center gap-1 rounded-full bg-brand-50 text-brand-700 px-2.5 py-1 mb-2">
          <Icon name="Lightning" size={12} /> <span className="text-[10px] font-bold">WHY 뷰티메이트</span>
        </div>
        <h2 className="text-[18px] font-extrabold text-gray-900">왜 뷰티메이트일까요?</h2>
        <div className="mt-4 grid grid-cols-2 gap-2.5">
      ...
      {/* ───── 라이브 뷰티 진행 방법 (고객 / 뷰티 전문가) ───── */}
      <section className="px-5 pt-8">
        <h2 className="text-[18px] font-extrabold text-gray-900">라이브 뷰티, 이렇게 진행돼요</h2>
        <p className="mt-1 text-[12px] text-gray-500">고객도, 뷰티 전문가도 쉽고 간단하게</p>
        <div className="mt-4 grid grid-cols-1 gap-2.5">
          ...
          <div className="rounded-2xl border border-brand-100 bg-brand-50/40 p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-[14px] font-bold text-gray-900">뷰티 전문가라면</p>
            </div>
            <ol className="space-y-2">
      ...
      <section className="px-4 pt-8">
        <div className="rounded-3xl border border-brand-100 bg-brand-50/50 p-5">
            ...
            {[
              {
                icon: CreditCard,
                label: "신용·체크카드",
                desc: "모든 카드사 즉시 결제. 할부도 지원돼 고객은 부담 없이, 뷰티 전문가는 결제 누락 걱정 없이 판매해요.",
              },
              {
                icon: Smartphone,
                label: "간편결제",
          ...
          <p className="mt-3.5 flex items-center gap-1.5 text-[12px] text-gray-600">
            ...
            카드·간편결제·계좌이체까지 모두 지원 — 뷰티 전문가는 그만큼 편하게 판매해요
          </p>
        </div>
      </section>
      ...
      <section className="px-5 pt-8 pb-2">
        ...
        <div className="mt-4 space-y-2.5">
          <StoryStep no={1} icon={<Icon name="Search" size={16} className="text-brand-600" />} title="뷰티 전문가를 찾아요" desc="뷰티 전문가 이름·분야로 검색해 마음에 드는 뷰티 전문가를 만나요." />
          <StoryStep no={2} icon={<Icon name="Wishlist" size={16} className="text-pink-500" />} title="뷰티 전문가를 단골로 설정해요" desc="단골로 설정하면 방송 시작 알림을 받고 예약 소식을 놓치지 않아요." />
        ...
        >
          ...
          <div className="relative p-5 text-white">
            <div className="inline-flex items-center gap-1 rounded-full bg-white/20 px-2 py-0.5 mb-2">
              ...
              <span className="text-[10px] font-bold">뷰티 전문가 모집</span>
            </div>
            ...
            <p className="mt-1.5 text-[12px] text-white/85 leading-relaxed">
              뷰티 서비스와 가능한 시간만 등록하면 오늘 방송부터 예약을 받을 수 있어요.
            </p>
            <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1.5 text-[11px] text-white/90">
              ...
              <span className="flex items-center gap-1"><Icon name="Sparkles" size={12} /> AI 고객노트</span>
            </div>
            <span className="mt-4 inline-flex items-center gap-1 rounded-full bg-white text-brand-700 text-[13px] font-bold px-4 py-2">
              뷰티 전문가로 신청하기 <Icon name="ChevronDown" size={15} className="-rotate-90" />
            </span>
          </div>
        </Link>
      </section>
      ...
      <section className="px-4 pt-8">
        <div className="rounded-3xl bg-gradient-to-br from-brand-50 to-amber-50 border border-brand-100 p-6 text-center">
          ...
          <h2 className="text-[16px] font-extrabold text-gray-900">뷰티메이트와 더 가까이</h2>
          <p className="text-[12px] text-gray-500 mt-1 leading-relaxed">
            새로운 뷰티 전문가와 라이브 소식,<br />이벤트를 가장 먼저 받아보세요.
          </p>
        ...
        <section className="px-4 pb-9">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[11px] text-gray-500 mt-0.5">뷰티 전문가 이름·코드로 진행 중인 라이브를 찾아보세요</p>
            </div>
      ...
      <section className="px-4 pt-2 pb-9">
        ...
        >
          ...
          <div className="relative">
            ...
            >
              뷰티 전문가 등록하기 <ArrowRight size={16} strokeWidth={2.4} />
            </Link>
