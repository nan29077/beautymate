    ...
    <div className="max-w-[480px] mx-auto px-4 py-6">
      ...
      <div className="mb-5">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="text-lg font-bold text-gray-900">패키지 상담상품</h1>
        </div>
        <p className="text-sm text-gray-500">여러 브랜드 상담상품을 한번에! 특가 패키지</p>
      </div>

      {packages.length === 0 ? (
        <div className="py-20 text-center">
          <Star size={48} className="mx-auto mb-4 opacity-20 text-gray-400" />
          <p className="text-sm text-gray-400">현재 판매 중인 패키지 상담상품이 없습니다.</p>
        </div>
      ) : (
                    ...
                    {pkg.items.slice(0, 3).map((item) => (
                      ...
                      >
                        {item.product.thumbnail ? (
                          ...
                          <Star size={14} className="m-auto mt-2.5 text-gray-300" />
                        )}
                      </div>
                    ))}
