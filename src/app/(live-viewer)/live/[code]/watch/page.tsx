...
import { Heart, Share2, Calendar, Send, Radio, ChevronDown, X, Minus, Plus, Clock, Eye, Video, Loader2, ChevronRight, Volume2, VolumeX, Star, HelpCircle, Gift, Info, Tag, Bell, BookOpen, ArrowLeft, Copy, Ticket, CheckCircle, ChevronUp, ExternalLink, Building2, Pin, Play, Pause, Sparkles } from "lucide-react";
...
export default function LiveWatchPage() {
  ...
  if (loading) return (
    <div className="fixed inset-0 flex items-center justify-center bg-[#0f0f1a]">
      <div className="text-center">
        <Sparkles size={80} strokeWidth={1.3} className="w-20 h-20 mx-auto mb-4 animate-pulse text-purple-300" aria-hidden="true" />
          ...
          <div key={coupon.id} className="relative overflow-hidden border-2 border-dashed rounded-2xl p-4"
            ...
            <div className="space-y-0.5 mb-3">
              ...
              {coupon.minOrderAmount != null && coupon.minOrderAmount > 0 && (
                <p className="text-[11px] text-gray-400">· 최소 예약금액: {coupon.minOrderAmount.toLocaleString()}원 이상</p>
              )}
  ...
  const renderBenefits = () => (
    <div className="space-y-3">
      {[
        { icon: Clock, label: "무료배송", desc: "라이브 상담상품 전상담상품 무료배송", color: "bg-blue-50 text-blue-500" },
          ...
          <button key={lp.id} onClick={() => handleBuyClick(lp.product.id)}
            ...
            <div className="relative flex-shrink-0">
              {lp.product.thumbnail
                ...
                : <div className="w-[72px] h-[72px] rounded-xl bg-gray-100 flex items-center justify-center border border-gray-100"><Star size={22} className="text-gray-300" /></div>
              }
              {lp.isActive && isLive && (
      ...
      <div>
        ...
        <div className="flex-shrink-0">
          {currentProduct.product.thumbnail
            ...
            : <div className="w-11 h-11 rounded-lg bg-gray-100 flex items-center justify-center"><Star size={14} className="text-gray-300" /></div>
          }
        </div>
        <div className="flex-1 min-w-0">
          ...
          <button key={lp.id} onClick={() => handleBuyClick(lp.product.id)}
            ...
            <div className="relative flex-shrink-0">
              {lp.product.thumbnail
                ...
                : <div className="w-[72px] h-[72px] rounded-xl bg-white/10 flex items-center justify-center border border-white/10"><Star size={22} className="text-white/30" /></div>
              }
              {lp.isActive && isLive && (
                <span className="absolute -top-1.5 -left-1.5 text-white text-[8px] font-bold px-1.5 py-0.5 rounded-md leading-none shadow-sm" style={{ backgroundColor: "#ff3b5c" }}>LIVE</span>
              )}
        ...
        return (
          <div key={coupon.id} className="relative overflow-hidden border-2 border-dashed rounded-2xl p-4"
            ...
            <div className="space-y-0.5 mb-3">
              ...
              {coupon.minOrderAmount != null && coupon.minOrderAmount > 0 && (
                <p className="text-[11px] text-white/30">· 최소 예약금액: {coupon.minOrderAmount.toLocaleString()}원 이상</p>
              )}
              {coupon.maxCount != null && (
                  ...
                  <button key={lp.id} onClick={() => { setShowMobilePurchase(false); setSelectedProductId(lp.product.id); }}
                    ...
                    <div className="relative flex-shrink-0">
                      {lp.product.thumbnail
                        ...
                        : <div className="w-16 h-16 rounded-lg bg-gray-100 flex items-center justify-center"><Star size={20} className="text-gray-300" /></div>
                      }
