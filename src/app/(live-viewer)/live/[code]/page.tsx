...
function BeeIcon({ size = 18 }: { size?: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none" aria-hidden="true" className="inline-block flex-shrink-0">
      ...
      <ellipse cx="16" cy="19" rx="10" ry="8.5" fill="#F5A623" />
...
export default function LiveChannelPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={HONEYCOMB_BG}>
        <div className="text-center">
          <Sparkles size={64} strokeWidth={1.3} className="w-16 h-16 mx-auto mb-3 animate-pulse" style={{ color: GOLD }} aria-hidden="true" />
  ...
  if (error || !channel) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6" style={HONEYCOMB_BG}>
        <Sparkles size={96} strokeWidth={1.2} className="w-24 h-24" style={{ color: GOLD }} aria-hidden="true" />
  ...
  const siteSettings: LiveSiteSettings = (() => {
    const defaults: LiveSiteSettings = {
      ...
      themeColor: "#F59E0B", buttonColor: "#F59E0B",
    };
  ...
  return (
    ...
    >
                  ...
                  <div className="absolute inset-0 overflow-hidden" style={{ backgroundColor: "#0d0d0d" }}>
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-1.5" style={{ zIndex: 3 }}>
                      <div
                        ...
                        style={{ color: "#F59E0B", animation: "tvBlink 1.4s step-end infinite" }}
                      >
                        ● 방송 대기 중
                      </div>
                      <div className="font-mono text-[9px] tracking-[0.5em] opacity-50" style={{ color: "#F59E0B" }}>
                        STANDBY
                      </div>
                    </div>
                  </div>
                  ...
                  <Link key={lp.id} href={`/products/${lp.product.id}`} className="w-[124px] flex-shrink-0">
                    <div className="relative w-[124px] h-[124px] rounded-xl overflow-hidden border border-amber-100 bg-amber-50">
                      {lp.product.thumbnail ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center"><Star size={24} className="text-amber-200" /></div>
                      )}
                      {lp.isActive && isLive && (
      ...
      {/* ═══ 예약내역 모달 ═══ */}
      <BottomSheet isOpen={showOrders} title="이 채널 예약내역" icon={<BookOpen size={15} style={{ color: GOLD }} />} onClose={() => setShowOrders(false)}>
        {ordersLoading ? (
          <SheetLoading />
        ) : ordersNeedLogin ? (
          <div className="py-8 text-center">
            ...
            <p className="text-[11px] text-gray-400 mt-1">로그인하고 예약 내역을 확인해보세요</p>
          ...
          <SheetEmpty message="이 채널에서 예약한 내역이 없어요" />
        ) : (
          <div className="space-y-2.5">
            {orders.map(o => (
              ...
              return (
                <Link key={lp.id} href={`/products/${lp.product.id}`} className="flex items-center gap-3 p-3 rounded-xl border border-amber-100 bg-white">
                  {lp.product.thumbnail ? (
                    ...
                    <div className="w-12 h-12 rounded-lg bg-amber-50 flex items-center justify-center flex-shrink-0"><Star size={16} className="text-amber-200" /></div>
                  )}
                  <div className="flex-1 min-w-0">
        ...
        {!channel.isLoggedIn ? (
          ...
          <div className="space-y-4 pb-2">
            ...
            {myOverview.orders.length > 0 && (
              <div>
                <p className="text-[12px] font-bold mb-2" style={{ color: BROWN }}>최근 예약</p>
                <div className="space-y-2">
...
function SheetEmpty({ message, sub }: { message: string; sub?: string }) {
  return (
    <div className="py-8 text-center">
      <Star size={34} className="text-amber-200 mx-auto" />
