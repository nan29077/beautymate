...
// PATCH: 상담 방식 상태 업데이트 (SUPER_ADMIN, BRAND_ADMIN, MIDDLE_ADMIN만 가능)
export async function PATCH(
  ...
  try {
    ...
    if (!allowedRoles.includes(role)) {
      return NextResponse.json({ error: "상담 방식 상태를 변경할 권한이 없습니다." }, { status: 403 });
    }
    ...
    if (deliveryStatus && !DELIVERY_STATUS_LABELS[deliveryStatus]) {
      return NextResponse.json({ error: "올바르지 않은 상담 방식 상태입니다." }, { status: 400 });
    }
    ...
    // 운송장 번호가 새로 입력/변경되면 고객에게 상담 방식 시작 알림톡 (실패해도 상태 저장에 영향 없음)
    ...
    if (newTracking && newTracking !== order.deliveryTracking) {
      await notifyShippingStartToBuyer(orderId).catch((e) =>
        console.error("[delivery] 상담 방식 시작 알림톡 오류:", e),
      );
    }
    ...
    return NextResponse.json({ error: "상담 방식 상태 업데이트에 실패했습니다." }, { status: 500 });
  }
}
