...
// 발주서(구매예약서) 개별 조회 API.
// 역할별 접근 권한 및 열람 가능한 상담상품 범위를 서버에서 제한한다.
//  - SUPER_ADMIN : 모든 예약의 전체 상담상품
//  - SELLER      : 자신의 점집에서 발생한 예약의 전체 상담상품
//  - BRAND_ADMIN : 자신이 등록한 상담상품(brandId=본인)이 포함된 예약 → 자신 상담상품만
//  - MIDDLE_ADMIN: 본인이 직접 등록한 상담상품(middleAdminId=본인) + 하위 브랜드 상담상품(brand.middleAdminId=본인) → 해당 상담상품만
export async function GET(_request: Request, { params }: { params: { id: string } }) {
  ...
  // 상담상품별 공급자(브랜드/중간관리자/상담사직접) 판별용 상담상품 정보 조회
  ...
  // ── 역할별 접근 권한 및 열람 가능 상담상품 판별 ──────────────────────────
  let canView = false;
  let visibleProductIds: Set<string> | null = null; // null = 전체 상담상품
  ...
  if (role === "SUPER_ADMIN") {
    ...
    if (middle) {
      // 본인이 직접 등록한 상담상품(middleAdminId=본인) + 하위 브랜드가 등록한 상담상품(brand.middleAdminId=본인)만 열람
      const ids = new Set(
        order.items
          .filter((i) => {
  ...
  const items = visibleItems.map((i) => {
    const p = productMap[i.productId];
    let supplierType = "상담사 직접";
    let supplierName = order.seller.shopName;
    // 브랜드 공급 상담상품이면 해당 브랜드를 관리하는 중간관리자명을 함께 표기
  ...
  return NextResponse.json({
    ...
    payment: {
      ...
      itemsTotal,
      // 브랜드/중간관리자는 일부 상담상품만 열람하므로 최종결제금액과 상담상품합계가 다를 수 있음
