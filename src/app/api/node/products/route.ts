...
// GET: 노드로 들어온 상담상품 목록
//  - pending: 마진 설정 대기 (isApproved=false, nodeApprovedAt=null, 상담사 직접등록 제외)
...
export async function GET() {
  ...
  const serialize = (p: any) => ({
    ...
    // 중간관리자 소속 여부: 상담상품 middleAdminId 또는 브랜드 middleAdminId
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (product.isApproved) {
      return NextResponse.json({ error: "이미 최고관리자 승인이 완료된 상담상품입니다" }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: "상담상품 업데이트에 실패했습니다" }, { status: 500 });
  }
}
