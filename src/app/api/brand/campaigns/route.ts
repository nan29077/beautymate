...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!campaignSellerId) {
      return NextResponse.json(
        { error: "등록된 상담사가 없어 단체 상담을 생성할 수 없습니다. 먼저 상담사를 승인해주세요." },
        { status: 400 }
      );
    }
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (!campaign || campaign.product.brandId !== brand.id) {
      return NextResponse.json({ error: "이 단체 상담에 대한 권한이 없습니다" }, { status: 403 });
    }
    ...
    if (action === "cancel") {
      ...
      return NextResponse.json({ success: true, message: "단체 상담이 취소되었습니다" });
    }
    ...
    if (action === "activate") {
      ...
      return NextResponse.json({ success: true, message: "단체 상담이 활성화되었습니다" });
    }
    ...
    if (action === "schedule") {
      ...
      return NextResponse.json({ success: true, message: "단체 상담이 예정으로 변경되었습니다" });
    }
    ...
    return NextResponse.json({ success: true, message: "단체 상담이 수정되었습니다" });
  } catch (error) {
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (orderCount > 0) {
      return NextResponse.json({
        error: `이 단체 상담에 ${orderCount}건의 예약이 있어 삭제할 수 없습니다. 취소 처리를 이용해주세요.`,
      }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: true, message: "단체 상담이 삭제되었습니다" });
  } catch (error) {
