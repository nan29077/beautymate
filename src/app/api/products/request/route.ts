...
// 상담상품 신청 목록 페이지당 기본/최대 개수
const DEFAULT_PAGE_SIZE = 20;
const MAX_PAGE_SIZE = 100;

// GET: 상담사가 신청할 수 있는 상담상품 목록 (서버 페이지네이션 + 검색/브랜드/제공방식 필터)
//
// 쿼리 파라미터
//   page       1-based 페이지 번호 (기본 1)
//   pageSize   페이지당 개수 (기본 20, 최대 100)
//   search     상담상품명 또는 브랜드명 부분 검색
//   brandId    브랜드 필터
...
export async function GET(req: NextRequest) {
  try {
    ...
    // 이미 점집에 담겨 있거나 신청한 상담상품은 목록에서 제외
    ...
    // 신청 대상: 관리자/브랜드가 등록(sellerId=null)하고 승인된 활성 상담상품만.
    // 다른 상담사가 직접 등록한 상담상품(sellerId != null)은 제외한다. (POST 검증과 동일 기준)
    const where: any = {
      isActive: true,
      isApproved: true,
      sellerId: null,
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 신청 가능한 상담상품인지 검증: 관리자/브랜드 등록(sellerId=null) + 승인 + 활성 상담상품만.
    // 다른 상담사가 직접 등록한 상담상품(sellerId != null)은 신청 불가.
    ...
    if (!product || !product.isActive || !product.isApproved || product.sellerId !== null) {
      return NextResponse.json({ error: "신청할 수 없는 상담상품입니다" }, { status: 400 });
    }

    // 상담사 입력 판매가 파싱 (공급가 제공 상담상품에서만 유효). 수수료 제공 상담상품은 판매가 고정.
    ...
    if (existing) {
      ...
      return NextResponse.json({ error: "이미 신청한 상담상품입니다." }, { status: 409 });
    }
