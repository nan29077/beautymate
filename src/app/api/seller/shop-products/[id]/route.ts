...
// 상담사 점집 상담상품 신청 취소 — 타인(브랜드/관리자) 등록 상담상품에 대한 '승인 대기' 신청건만 삭제.
// 본인 등록 상담상품(자동 승인) 및 이미 승인된 판매 상담상품은 삭제 대상이 아니다.
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  // 본인이 직접 등록한 상담상품(자동 승인)은 신청 취소 대상이 아니다.
  if (shopProduct.product.sellerId === seller.id) {
    return NextResponse.json({ error: "본인 등록 상담상품은 신청 취소할 수 없습니다." }, { status: 400 });
  }
  // 이미 승인된 판매 상담상품은 취소가 아닌 판매관리(삭제)로 처리한다.
  if (shopProduct.isApproved) {
    return NextResponse.json({ error: "이미 승인된 상담상품은 신청 취소할 수 없습니다." }, { status: 400 });
  }
