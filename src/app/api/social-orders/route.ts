...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ error: "소셜예약서 조회에 실패했습니다." }, { status: 500 });
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    return NextResponse.json({ error: "소셜예약서 삭제에 실패했습니다." }, { status: 500 });
  }
}
