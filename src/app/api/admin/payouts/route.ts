...
export async function POST(req: NextRequest) {
  ...
  // 지급 직전 가용 잔액 재검증 — 출금 신청 이후 예약 취소/환불로 정산액이
  ...
  if (action === "pay") {
    ...
    if (Number(payout.amount) > remaining) {
      return NextResponse.json(
        {
          error: `지급 시 가용 잔액이 부족합니다. (출금액 ${Number(payout.amount).toLocaleString()}원 > 잔액 ${Math.max(0, remaining).toLocaleString()}원 — 신청 후 예약 취소 등으로 정산액이 감소했을 수 있습니다.)`,
        },
        { status: 409 },
      );
    }
