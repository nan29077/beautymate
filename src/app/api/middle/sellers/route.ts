...
export async function POST(request: Request) {
  ...
  try {
    ...
    // 유저 + 상담사 프로필 동시 생성 (본인 소속으로 연결, 즉시 승인)
    ...
    const user = await prisma.user.create({
      data: {
        ...
        sellerProfile: {
          create: {
            slug,
            shopName: shopName?.trim() || `${name}의 점집`,
            isApproved: true,
    ...
    return NextResponse.json({ error: "상담사 등록에 실패했습니다." }, { status: 500 });
  }
}
