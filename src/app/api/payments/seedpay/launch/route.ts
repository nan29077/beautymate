...
export async function GET(request: Request) {
  ...
  if (!order) return errorPage("예약을 찾을 수 없습니다.");
  if (order.userId !== session.user!.id) return errorPage("본인 예약만 결제할 수 있습니다.");
  if (order.paymentStatus === "COMPLETED") return errorPage("이미 결제 완료된 예약입니다.");
