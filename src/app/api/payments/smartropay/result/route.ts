...
// 인증 결과(Tid + TrAuthKey)를 POST 로 받아 승인 API 호출 후 예약 상태 업데이트.
export async function POST(request: Request) {
  ...
  // 멱등성 가드: 이미 결제완료(COMPLETED)된 예약은 중복 콜백으로 다시 들어와도
  ...
  try {
    ...
    await notifyOrderPlacedToSeller(order.id).catch((e) => console.error("[smartropay] 예약접수 알림톡 오류:", e));
    await logPayment({
      orderId: order.id,
      provider: "smartropay",
      stage: "approval",
