...
export async function POST(request: Request) {
  ...
  if (!pkg) {
    return NextResponse.json({ error: "패키지 상담상품을 찾을 수 없습니다." }, { status: 404 });
  }

  if (pkg.status !== "APPROVED") {
    return NextResponse.json({ error: "승인된 패키지 상담상품만 구매할 수 있습니다." }, { status: 400 });
  }
