...
export async function POST(req: NextRequest) {
  ...
  if (existing) {
    ...
    return NextResponse.json({
      ...
      message: "인증 재신청 완료. 상담사/관리자 확인 후 승인됩니다.",
    });
  }
  ...
  return NextResponse.json({
    verification,
    message: "채널 인증 신청 완료. 상담사/관리자 확인 후 승인됩니다.",
  });
}

// PATCH: 인증 승인/거부 (상담사 또는 관리자)
export async function PATCH(req: NextRequest) {
  ...
  if (action === "approve") {
    ...
    // 거절 시 Pick(팔로우) 해제 + totalFans 차감 (인증 기록은 REJECTED로 유지)
