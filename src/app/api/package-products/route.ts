...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!items || !Array.isArray(items) || items.length < 2) {
      return NextResponse.json({ error: "구성 상담상품은 최소 2개 이상 선택해야 합니다." }, { status: 400 });
    }

    // 구성 상담상품 유효성 검증 (존재하지 않는 productId로 인한 FK 오류 방지)
    const productIds = items.map((item: { productId: string }) => item.productId).filter(Boolean);
    if (productIds.length !== items.length) {
      return NextResponse.json({ error: "구성 상담상품 정보가 올바르지 않습니다." }, { status: 400 });
    }
    const existingCount = await prisma.product.count({ where: { id: { in: productIds } } });
    if (existingCount !== new Set(productIds).size) {
      return NextResponse.json({ error: "존재하지 않는 상담상품이 포함되어 있습니다. 구성 상담상품을 다시 선택해주세요." }, { status: 400 });
    }
    ...
    console.error("패키지 상담상품 등록 오류:", error);
    ...
    if (
      ...
      return NextResponse.json(
        { error: "패키지 상담상품 DB 테이블이 없습니다. 'npx prisma db push'를 실행해 주세요." },
        { status: 503 }
      );
    }
    return NextResponse.json(
      { error: "패키지 상담상품 등록에 실패했습니다. 잠시 후 다시 시도해주세요." },
      { status: 500 }
    );
  }
}
