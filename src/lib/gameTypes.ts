...
// 항목(items) 배열을 상담사가 직접 입력해두는 게임 (참여자 불필요) — 타입 A
...
// (하위 호환) 상담사가 직접 확률 뽑기를 실행하던 게임 — 현재 BOX_OPEN 은 참여형으로 이동
...
export const GAME_TYPE_GUIDE: Record<GameTypeId, string> = {
  ...
  DRAW: "항목들 중 하나를 무작위로 뽑습니다. 선물, 쿠폰, 상담상품 등을 넣어 추첨하세요.",
  SEQUENTIAL:
  ...
  QUIZ:
    "객관식 퀴즈 게임입니다. 진행자가 문제를 내면 시청자들이 1~4번 중 정답을 선택합니다. 모든 답변이 집계된 후 정답률과 당첨자를 확인할 수 있습니다. 참여 방법: QR코드로 접속 → 선택지 터치. 운영 팁: 상담상품 관련 퀴즈로 구매 욕구를 자극하세요.",
  VOTE:
    "시청자 의견을 실시간으로 수집하는 투표 게임입니다. 이름 없이 익명으로 참여하며, 투표 결과를 오버레이로 실시간 표시합니다. 활용 예: '어떤 색상이 더 좋으세요?', '다음 상담상품은 무엇을 원하시나요?' 등 시청자 참여도를 높이는 데 효과적입니다.",
  NUMBER_GUESS:
    "정해진 범위 내에서 숫자를 맞추는 게임입니다. 시청자들이 각자 예상 숫자를 입력하고, 정답에 가장 가까운 시청자가 당첨됩니다. 참여 방법: QR코드로 접속 → 숫자 직접 입력. 활용 예: '이 상담상품의 가격은?', '오늘 라이브 시청자 수는?' 등 긴장감 있는 게임 진행이 가능합니다.",
  KEYWORD:
    "진행자가 힌트를 주면 시청자가 키워드를 직접 입력하는 게임입니다. 정확히 일치하는 키워드를 입력한 시청자 중 추첨으로 당첨자를 선정합니다. 참여 방법: QR코드로 접속 → 키워드 직접 입력. 활용 예: 상담상품명, 브랜드명, 이벤트 해시태그 등을 맞추는 방식으로 브랜드 인지도를 높일 수 있습니다.",
  BOX_OPEN:
    "시청자들이 박스를 열어 당첨 여부를 확인하는 랜덤 게임입니다. 박스 구성(상담상품/쿠폰/꽝)과 확률을 직접 설정하고, 시청자가 버튼 클릭 시 즉시 결과가 결정됩니다. 참여 방법: QR코드로 접속 → '박스 열기' 버튼 클릭. 운영 팁: 당첨 확률을 낮게 설정하면 희소성이 높아집니다.",
  GOAL_GAUGE:
...
// 상담사가 '결과 발표'로 당첨자를 집계하는 게임 (선착순/정답/투표 등)
...
export function defaultConfig(type: string): Record<string, unknown> {
  switch (type) {
    ...
    case "BOX_OPEN":
      return {
        boxes: [
          { label: "1등 상담상품", kind: "PRODUCT", prob: 10 },
