...
// - 뷰티 서비스 썸네일 없음 → 뷰티메이트 브랜드 placeholder(자체 SVG)
// - 뷰티샵 상단 배경 기본값 → 깔끔한 실사 이미지(unsplash)
// - 역할별 프로필 기본 이미지 → 역할별 캐릭터 아바타(public/avatars/*.png)

// 뷰티메이트 전용 "노이미지" placeholder (public/no-image.png — 노랑+검정 브랜드 톤)
// 뷰티 서비스 썸네일이 없거나(null/빈 문자열) 로드 실패할 때 모든 화면에서 이 이미지를 표시.
export const NO_IMAGE = "/no-image.png";

// 기존 뷰티 서비스 placeholder 상수는 NO_IMAGE 로 통일 (레거시 참조 호환용)
export const DEFAULT_PRODUCT_IMAGE = NO_IMAGE;

// 뷰티샵 상단 기본 배너 (깔끔한 실사)
export const DEFAULT_SHOP_BANNER = "/banners/beautymate/hero-salon-v3.png";

// ─── 역할별 캐릭터 아바타 (public/avatars/*.png) ────────────────────────────

// 최고관리자 (5종)
export const ADMIN_AVATARS = ["/avatars/beautymate/default.svg"];

// 중간관리자·노드 (5종)
export const MIDDLE_ADMIN_AVATARS = ["/avatars/beautymate/default.svg"];

// 브랜드사 (6종)
export const BRAND_AVATARS = ["/avatars/beautymate/default.svg"];

// 고객 — 여성(13종), 남성(13종)
export const BUYER_FEMALE_AVATARS = ["/avatars/beautymate/default.svg"];
export const BUYER_MALE_AVATARS = ["/avatars/beautymate/default.svg"];
export const ALL_BUYER_AVATARS = [...BUYER_FEMALE_AVATARS, ...BUYER_MALE_AVATARS];

// 뷰티메이트 랜덤 동물 캐릭터(30종) — 십이지신 계열 동물 + 한복 + 밤하늘/달·별 일러스트.
// 일반 가입 고객·최고관리자·뷰티 전문가가 모두 이 풀을 공유한다.
// 뷰티 전문가 개인 샵(?ref=<slug>)으로 가입한 고객은 기존 구매회원 캐릭터 풀을 그대로 사용한다.
export const BEAUTYMATE_CUSTOMER_AVATARS = ["/avatars/beautymate/default.svg"];

// 뷰티 전문가 캐릭터 풀 — 뷰티 동물 캐릭터를 그대로 사용한다.
// (뷰티메이트 시절 꿀벌 캐릭터 /avatars/라이브셀러_*.png 는 폐기)
export const SELLER_AVATARS = BEAUTYMATE_CUSTOMER_AVATARS;
...
export function pickRoleAvatar(seed: string, role: string, gender?: string | null): string {
  ...
  switch (role) {
    case "SUPER_ADMIN":
      return pickBeautyMateAvatar(`admin:${seed}`);
    case "CONSULTANT":
...
export function pickBeautyMateAvatar(seed: string): string {
  return BEAUTYMATE_CUSTOMER_AVATARS[computeHash(seed) % BEAUTYMATE_CUSTOMER_AVATARS.length];
}

export function randomBeautyMateAvatar(): string {
  return BEAUTYMATE_CUSTOMER_AVATARS[Math.floor(Math.random() * BEAUTYMATE_CUSTOMER_AVATARS.length)];
}

// 뷰티메이트 시절 번들 캐릭터(/avatars/*.png — 꿀벌 뷰티 전문가·구매회원 등) 여부.
// /avatars/beautymate/* 는 뷰티메이트 전용 캐릭터이므로 레거시가 아니다.
export function isLegacyBundledAvatar(path?: string | null): boolean {
  return Boolean(path?.startsWith("/avatars/") && !path.startsWith("/avatars/beautymate/"));
}

// 관리자 화면에서는 기존 번들 기본 캐릭터를 새 뷰티 캐릭터로 교체한다.
...
// 뷰티 전문가 프로필 이미지 확정.
// 직접 업로드한 이미지(뷰티샵 로고 · /uploads 등)는 보존하고, DB 에 남아 있는 레거시 꿀벌
// 캐릭터 경로이거나 이미지가 아예 없으면 seed 기반 뷰티 동물 캐릭터로 교체한다.
export function resolveConsultantAvatar(seed: string, currentAvatar?: string | null): string {
  if (currentAvatar && !isLegacyBundledAvatar(currentAvatar)) return currentAvatar;
  return pickBeautyMateAvatar(seed);
}

// ─── 뷰티샵 기본 배너 (생성형 이미지 5종) ──────────────────────────────────────
// 배너를 올리지 않은 뷰티 전문가에게 뷰티메이트의 보라·금빛 분위기를 담은 기본 배너를 배정한다.
// 뷰티 전문가 id 해시 기반이라 같은 뷰티 전문가는 항상 같은 배너를 본다.
export const SHOP_BANNERS = [
  "/banners/beautymate/hero-live-v3.png",
  "/banners/beautymate/hero-color-v3.png",
  "/banners/beautymate/hero-salon-v3.png",
];
...
/** 뷰티샵 배너 확정 — 업로드한 배너가 있으면 그대로, 없으면 seed 기반 기본 배너. */
...
// ─── 뷰티 전문가 표시 이미지 단일 진입점 ─────────────────────────────────────────
// 앱 전체에서 뷰티 전문가 프로필 이미지는 반드시 이 함수로 구한다.
// (예전에는 화면마다 pickSellerAvatar(seller.id) / pickSellerAvatar(seller.slug) 처럼
//  seed 가 제각각이라 같은 뷰티 전문가가 화면마다 다른 캐릭터로 보였다.)
//
// 우선순위: 뷰티샵 로고 > 회원 아바타(가입 시 배정된 동물 캐릭터) > sellerId 해시 동물 캐릭터
...
export function resolveSellerDisplayImage(seller: SellerImageInput, seedFallback?: string): string {
  ...
  return pickBeautyMateAvatar(seller.id || seedFallback || "seller");
}
