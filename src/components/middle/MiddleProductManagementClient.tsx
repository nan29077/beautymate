    ...
    <div className="animate-fade-in space-y-5">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-shrink-0 flex-wrap justify-end">
          ...
          <ProductRegisterForm brands={[]} mode="middle" buttonLabel="내 상담상품 등록" hideGroupBuy />
        </div>
      </div>

      <div className="bg-amber-50 border border-amber-100 rounded-xl p-3.5 text-[12px] text-amber-700 leading-relaxed">
        브랜드가 등록한 상담상품에 <b>판매가</b>와 <b>마진</b>을 입력 후 저장하면, <b>최고관리자 승인</b> 후 상담사에게 노출됩니다.
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20 text-gray-400 bg-white rounded-xl border border-gray-100">
          ...
          <p className="text-sm">표시할 상담상품이 없습니다.</p>
        </div>
      ) : (
              ...
              <div key={p.id} className="bg-white rounded-xl border border-gray-100 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {p.isOwn && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded-full font-medium bg-blue-50 text-blue-600">내 등록상담상품</span>
                      )}
                      {isSelling && (
                ...
                {p.isOwn ? (
                  <div className="mt-3 text-[11px] text-gray-500 bg-amber-50/60 border border-amber-100 rounded-lg px-3 py-2 leading-relaxed">
                    {p.isApproved
                      ? "최고관리자 승인이 완료되어 상담사에게 노출됩니다."
                      : "최고관리자 승인 후 상담사의 상담상품 신청 목록에 노출됩니다."}
                  </div>
                ) : (
                  <>
                    ...
                    <div className="mt-3 flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-[11px] text-gray-400 flex items-center gap-0.5">
                        ...
                        {p.isApproved ? "최고관리자 승인 완료 · 상담사 노출 중" : "저장 후 최고관리자 승인 시 상담사에게 노출됩니다"}
                      </span>
                    </div>
                  </>
                )}
