...
/** 방송 카드 썸네일: 방송 썸네일 → 첫 상담상품 썸네일 → 상담사 프로필 이미지 폴백 */
...
export default function SellerShopTabs({
  ...
  // 지난 방송 상담상품 탭은 showPastInShop=true인 종료 방송이 있을 때만 표시.
  // manualPastProducts(isManualLive OFF 시 자동 이동)는 탭 자체의 노출 여부를 결정하지 않음.
  // — 상담사가 PastBroadcastToggles에서 모두 OFF 하면 탭이 사라지는 것이 올바른 동작.
  const showPastTab = endedLives.length > 0;

  const tabs = [
    ...(directExpose && products.length > 0 ? [{ id: "products", label: "상담상품", count: products.length }] : []),
    ...(showLiveTab ? [{ id: "live", label: "현재 라이브 중 상담상품", count: (currentLive ? 1 : 0) + scheduledLives.length, isLive: isLive || manualLiveProducts.length > 0 }] : []),
    ...(showPastTab ? [{ id: "past", label: "지난 방송 상담상품", count: endedLives.length }] : []),
    ...(features.groupBuy && campaigns.length > 0 ? [{ id: "campaigns", label: "단체 상담", count: campaigns.length }] : []),
    ...(directExposureOn ? [{ id: "direct", label: "일반상담상품", count: directProducts.length }] : []),
  ];
  ...
  if (tabs.length === 0) {
    return (
      <div className="mt-4 px-4 py-16 text-center text-gray-400 bg-gray-50 rounded-xl mx-4">
        ...
        <p className="text-xs">아직 준비 중인 점집입니다</p>
      </div>
    );
  }
      ...
      <div className="px-4 py-5">
        ...
        {activeTab === "live" && (
          <div className="space-y-6 animate-fade-in">
            {/* 수동 라이브 노출 상담상품 (isManualLive ON 상태, 라이브 상담 없을 때만) */}
            ...
            {currentLive && (
              <div>
                ...
                {currentLive.products.length > 0 ? (
                  ...
                  <p className="text-xs text-gray-400 text-center py-4">등록된 상담상품이 없습니다</p>
                )}
              </div>
            )}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {live.products.length > 0 && (
                          <p className="text-[10px] text-gray-400 mt-0.5">상담상품 {live.products.length}개</p>
                        )}
                      </div>
        ...
        {activeTab === "past" && (
          <div className="space-y-3 animate-fade-in">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h2 className="text-[15px] font-bold text-gray-900">지난 방송 상담상품</h2>
                  ...
                  >
                    <div className="relative w-20 h-20 rounded-xl overflow-hidden bg-amber-100 flex-shrink-0">
                      {live.thumbnailImage ? (
                        ...
                        <>
                          <div className="w-full h-full flex items-center justify-center bg-[#2d1b69]">
                            <Sparkles size={18} strokeWidth={1.4} className="text-white/70" aria-hidden="true" />
                          </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1.5">
                        <span className="text-[10px] text-gray-400 flex items-center gap-0.5">
                          <Icon name="Video" size={10} /> 상담상품 {live.products.length}개
                        </span>
                  ...
                  {isOpen && (
                    <div className="border-t border-gray-50 px-3 pb-3 pt-1 animate-fade-in">
                      {live.products.length > 0 ? (
                        ...
                        <p className="text-[10px] text-gray-400 py-4 text-center">등록된 상담상품이 없습니다</p>
                      )}
                    </div>
                  )}
                </div>
                ...
                >
                  <div className="aspect-[4/5] overflow-hidden bg-gray-100 relative">
                    <SafeImage
                      ...
                      width={220}
                      height={275}
                      fallbackText={campaign.product?.name || "상담상품"}
          ...
          <div className="animate-fade-in">
            {directProducts.length === 0 ? (
              <div className="text-center py-16 text-gray-400 bg-white rounded-xl border border-gray-100">
                ...
                <p className="text-xs">등록된 상담상품이 없습니다</p>
              </div>
            ) : (
            <div className="grid grid-cols-2 gap-3">
