...
import { Sparkles, Printer, Loader2, X, User as UserIcon, Phone, MapPin, Tag, Landmark, List, Youtube, Instagram, AtSign, Star, Trash2 } from 'lucide-react';
import OrderManagementClient from "@/components/shared/OrderManagementClient";
import { useAppDialog } from "@/components/shared/AppDialog";
import Pagination, { usePagination } from "@/components/shared/Pagination";

// 테마 캘린더 아이콘 — lucide 컴포넌트 자리(DetailRow icon)에 그대로 사용 가능
      ...
      <div className="relative w-full max-w-[360px] bg-white rounded-t-2xl sm:rounded-2xl shadow-xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-1.5">
            ...
            <h2 className="text-[15px] font-bold text-gray-900">상담상품 상세</h2>
          </div>
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-1 mb-5 bg-amber-50/60 p-1 rounded-xl w-full sm:w-fit border border-amber-100">
        ...
        >
          <Icon name="SnsDm_icon" size={15} /> 계좌 입금 예약서
...
function SocialOrderPanel({
  ...
  const handleDeleteSelected = async () => {
    if (deleting) return;
    if (selected.size === 0) {
      showToast("삭제할 계좌 입금 예약서를 선택해주세요.");
      return;
    }
    const ids = Array.from(selected);
    const ok = await appConfirm({
      title: "정말로 삭제하시겠습니까?",
      message: `선택한 계좌 입금 예약서 ${ids.length}건을 삭제합니다.\n삭제된 예약서는 복구할 수 없어요.`,
      type: "confirm",
      confirmText: "삭제",
      cancelText: "취소",
    });
  ...
  const handleDeleteOne = async (order: SocialOrder) => {
    ...
    const ok = await appConfirm({
      title: "정말로 삭제하시겠습니까?",
      message: `'${order.name}' 님의 계좌 입금 예약서를 삭제합니다.\n삭제된 예약서는 복구할 수 없어요.`,
      type: "confirm",
      confirmText: "삭제",
      cancelText: "취소",
    });
  ...
  const downloadExcel = async () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "계좌 입금 예약서");
    const today = new Date().toISOString().slice(0, 10);
    XLSX.writeFile(wb, `계좌입금예약서_${sellerName}_${today}.xlsx`);
  };

  // PDF 출력: 각 소셜예약서를 한 장(page-break)으로 구성 → 브라우저 window.print()로 인쇄/PDF 저장
  const downloadPDF = () => {
  ...
  if (orders.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-sm font-semibold text-gray-500">접수된 계좌 입금 예약서가 없습니다</p>
        <p className="text-xs text-gray-400 mt-1">고객이 상담상품 페이지에서 작성한 계좌 입금 예약서가 여기에 표시됩니다</p>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap mb-4">
        <div className="flex items-start gap-2 bg-amber-50 border border-amber-100 rounded-xl px-4 py-3 flex-1 min-w-[200px]">
          ...
          <div>
            <p className="text-[13px] font-bold text-amber-800">계좌 입금 예약서 {orders.length}건이 접수되었습니다.</p>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-[11px] text-gray-400 uppercase tracking-wide">
              ...
              <th className="text-left px-3 py-3 font-semibold">상담상품명</th>
                ...
                <td className="text-center px-3 py-3">
                  <div className="flex flex-col items-center gap-1.5">
                    <div className="flex items-center gap-1">
                      ...
                      >
                        <Icon name="Package" size={11} className="shrink-0" /> 상담상품
                      </button>
                      <button
                        onClick={() => setDetail(o)}
                        className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-gray-50 hover:bg-amber-50 border border-gray-200 text-gray-600 text-[11px] font-semibold whitespace-nowrap transition-colors"
        ...
        {orders.map((o, i) => (
          <div key={o.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              ...
              >
                <Icon name="Package" size={13} /> 상담상품 상세
              </button>
              <button
                onClick={() => setDetail(o)}
                className="inline-flex items-center justify-center gap-1 px-3 py-2 rounded-lg bg-gray-50 hover:bg-amber-50 border border-gray-200 text-gray-600 text-[12px] font-semibold transition-colors"
              >
                <Icon name="Eye" size={13} /> 예약서 상세
              </button>
            </div>
            <div className="mt-2">
              {(confirmedIds.has(o.id) || o.status === "CONFIRMED") ? (
      ...
      <div className="relative w-full max-w-[440px] bg-white rounded-t-2xl sm:rounded-2xl shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white z-10 flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-1.5">
            ...
            <h2 className="text-[15px] font-bold text-gray-900">계좌 입금 예약서 상세</h2>
          </div>
          <button onClick={onClose} className="p-1 text-gray-400 hover:text-gray-700">
            <X size={20} />
          </button>
        ...
        <div className="px-5 py-4 space-y-3.5">
          <DetailRow icon={Star} label="상담상품명" value={order.productName} />
          ...
          {/* 고객 SNS 정보 — 구매 시 입력한 채널 아이디 / 대화명 (있을 때만) */}
          {(order.snsHandle || order.snsNickname) && (
            <div className="rounded-xl border border-amber-100 bg-amber-50/50 px-3.5 py-3">
              <p className="text-[11px] font-semibold text-amber-600 mb-2 flex items-center gap-1">
                <Icon name="Message" size={12} strokeWidth={1.75} /> 고객 SNS 정보
              </p>
...
// 소셜예약서 일괄 출력/모아보기용 HTML (autoPrint=true 시 창 로드 후 자동 인쇄)
function buildSocialSheetsHtml(orders: SocialOrder[], sellerName: string, autoPrint: boolean): string {
  ...
  const sheets = orders
    .map((o, i) => {
      ...
      <section class="sheet${i < orders.length - 1 ? " page-break" : ""}">
        ...
        <table class="meta">
          <tr><th>제출일시</th><td>${escapeHtml(formatDateTime(o.createdAt))}</td><th>상담사</th><td>${escapeHtml(sellerName)}</td></tr>
          <tr><th>고객명</th><td>${escapeHtml(o.name)}</td><th>연락처</th><td>${escapeHtml(o.phone)}</td></tr>
          <tr><th>입금자명</th><td colspan="3">${escapeHtml(o.depositorName || o.name)}</td></tr>
        </table>

        <h2>예약 상담상품</h2>
        <table class="items">
          <thead><tr><th>상담상품명</th><th style="width:140px">가격</th></tr></thead>
          <tbody>
  ...
  return `<!doctype html>
<html lang="ko"><head><meta charset="utf-8"><title>계좌 입금 예약서 일괄 출력</title>
...
<body>
  ...
  ${sheets || '<div class="sheet"><p style="text-align:center;color:#9ca3af">출력할 계좌 입금 예약서가 없습니다.</p></div>'}
  ${autoPrint ? "<script>window.onload = function(){ setTimeout(function(){ window.print(); }, 300); };<\/script>" : ""}
</body></html>`;
}

// 장바구니 일괄 출력/모아보기용 HTML — 고객별 담은 상담상품 목록
  ...
  const sheets = groups
    .map((g, i) => {
      ...
      <section class="sheet${i < groups.length - 1 ? " page-break" : ""}">
        ...
        <table class="meta">
          <tr><th>고객명</th><td>${escapeHtml(g.userName || "이름없음")}</td><th>이메일</th><td>${escapeHtml(g.userEmail)}</td></tr>
          <tr><th>상담사</th><td colspan="3">${escapeHtml(sellerName)}</td></tr>
        </table>

        <h2>담은 상담상품</h2>
        <table class="items">
          <thead><tr><th>상담상품명</th><th style="width:120px">옵션</th><th style="width:50px">수량</th><th style="width:100px">단가</th><th style="width:150px">담은 날짜</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </section>`;
    })
...
function CartPanel({
  ...
  const sendBulk = () => {
    if (selected.size === 0) return showToast("발송할 고객을 선택해주세요.");
  ...
  const handleDeleteSelected = async () => {
    ...
    // 선택된 고객 그룹에 속한 모든 장바구니 항목 id 수집
    const ids = groups
      .filter((g) => selected.has(g.userId))
      .flatMap((g) => g.items.map((i) => i.id));
    const ok = await appConfirm({
      title: "정말로 삭제하시겠습니까?",
      message: `선택한 고객 ${selected.size}명의 장바구니 상담상품 ${ids.length}건을 삭제합니다.\n삭제된 항목은 복구할 수 없어요.`,
      type: "confirm",
      confirmText: "삭제",
      cancelText: "취소",
    });
  ...
  const handleDeleteGroup = async (group: { userId: string; userName: string; items: CartItem[] }) => {
    ...
    const ok = await appConfirm({
      title: "정말로 삭제하시겠습니까?",
      message: `'${group.userName || "이름없음"}' 님의 장바구니 상담상품 ${ids.length}건을 삭제합니다.\n삭제된 항목은 복구할 수 없어요.`,
      type: "confirm",
      confirmText: "삭제",
      cancelText: "취소",
    });
  ...
  if (cartItems.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-sm font-semibold text-gray-500">장바구니에 담긴 상담상품이 없습니다</p>
        <p className="text-xs text-gray-400 mt-1">고객이 담아둔 상담상품이 여기에 표시됩니다</p>
      </div>
    );
  }
      ...
      <div className="flex items-start gap-2 bg-amber-50 border border-amber-100 rounded-xl px-4 py-3 mb-4">
        ...
        <div>
          ...
          <p className="text-[11px] text-amber-700/80 mt-0.5">
            고객에게 알림톡을 보내 구매를 유도하세요. (알림톡 연동은 준비 중입니다)
          </p>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-[11px] text-gray-400 uppercase tracking-wide">
              ...
              <th className="text-left px-3 py-3 font-semibold">고객명/닉네임</th>
              <th className="text-left px-3 py-3 font-semibold">상담상품명</th>
              ...
              g.items.map((c, idx) => (
                <tr key={c.id} className="border-t border-gray-50 hover:bg-amber-50/30">
                  ...
                  <td className="px-3 py-3">
                    ...
                    >
                      <Icon name="Package" size={11} className="shrink-0" /> 상담상품 상세
                    </button>
                  </td>
