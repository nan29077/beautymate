...
import { Star, BookOpen, Radio, Loader2, X, LayoutList, Filter, MessageCircle, Ban, PencilLine, XCircle } from 'lucide-react';
...
export default function SellerProductTabs({ data }: { data: TabData }) {
  ...
  // 상담상품신청 메뉴 노출은 최고관리자 권한설정(productRequest 토글)을 따른다.
  const flags = useFeatureFlags();
  // 단일 5탭: 승인대기 / 판매중 / 판매중지 / 상담상품 신청 / 일반상담상품
  ...
  // 해당 상담상품이 상담사 본인이 직접 등록한 상담상품인지 판별
  ...
  // ★ 상담상품 신청 탭 — 검색/필터/페이지네이션 모두 서버(GET /api/products/request)에서 처리한다.
  //    전체 목록을 클라이언트로 내려받지 않으므로 상담상품 수가 늘어나도 페이지 이동이 정상 동작한다.
  ...
  // 내 상담상품 목록 (AvailableProduct 형태로 변환)
  ...
  const handleApplyProduct = async (productId: string, type: "groupbuy" | "live", isOwn: boolean) => {
    ...
    try {
      ...
      if (isOwn) {
        await appAlert(`${type === "groupbuy" ? "단체 상담" : "라이브 상담"} 상담상품이 추가되었습니다.`);
      } else {
        await appAlert(`${type === "groupbuy" ? "단체 상담" : "라이브 상담"} 상담상품 신청이 완료되었습니다. 브랜드 승인 후 판매가 가능합니다.`);
      }
  ...
  const handleAddToMyShop = async (productId: string) => {
    ...
    try {
      ...
      // 라이브 진행 중 + 해당 상담상품이 라이브에 포함되어 있으면 라이브 메시지
  ...
  const handleShopProductAction = async (shopProductId: string, productId: string, action: "pauseSale" | "startSale" | "delete") => {
    if (action === "delete") {
  ...
  // ★ 신청 취소 (승인 대기 중인 타인 등록 상담상품 신청건 삭제)
  const [cancelLoading, setCancelLoading] = useState<string | null>(null);
  const handleCancelRequest = async (shopProductId: string) => {
    if (!await appConfirm({ message: "상담상품 신청을 취소하시겠습니까?", type: "warning", confirmText: "신청 취소" })) return;
  ...
  // 단일 5탭 구성 (상담상품 신청 탭은 최고관리자 권한설정 토글에 따라 노출)
  const tabItems: { key: typeof tab; label: string; count: number | null }[] = [
    ...
    ...(flags.productRequest ? [{ key: "request" as const, label: "상담상품 신청", count: availableProductsCount ?? 0 }] : []),
    { key: "direct", label: "일반상담상품(빠른상담상품 등록)", count: directProductCount ?? null },
  ];
        ...
        <div className="space-y-5 animate-fade-in">
          ...
          {tab === "paused" && (filteredPaused.length > 0 ? (
                      ...
                      <div className="p-2">
                        ...
                        <div className="mt-1 flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              onClick={() => handleShopProductAction(sp.id, sp.product.id, "delete")}
                        ...
                        {/* ★ 상담사 본인 등록 상담상품: 상담상품수정 버튼 (카드 하단) */}
                        {isOwnProduct(sp) && (
                          <button
            ...
            <div className="text-center py-16 text-gray-400 bg-white rounded-xl border border-gray-100">
              ...
              <p className="text-sm">판매중지된 상담상품이 없습니다.</p>
            </div>
          ))}
          ...
          {tab === "pending" && (filteredPending.length === 0 && filteredRejected.length === 0 ? (
            <div className="text-center py-16 text-gray-400 bg-white rounded-xl border border-gray-100">
              <Icon name="Package" size={40} strokeWidth={1.5} className="mx-auto mb-3 opacity-30" />
              <p className="text-sm">승인 대기 중인 상담상품이 없습니다.</p>
            </div>
          ) : (
          <>
          ...
          {tab === "active" && (filteredActive.length > 0 ? (
            <div className="bg-green-50 border border-green-200 rounded-xl p-3">
              <div className="flex items-center gap-2 mb-2.5">
                ...
                <h2 className="text-sm font-bold text-green-700">판매중 상담상품</h2>
                      ...
                      <div key={sp.product.id} className="flex items-center gap-2.5 sm:gap-3 p-3 sm:p-4 hover:bg-gray-50 transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-1.5 sm:gap-2 mt-0.5 flex-wrap">
                            ...
                            <span className="text-[10px] text-gray-400">후기 {sp.product.reviewCount}</span>
                          </div>
                        </div>
                        <div className="text-right flex-shrink-0">
                          <p className="text-[13px] sm:text-sm font-semibold">{formatPrice(sp.product.basePrice)}</p>
            ...
            <div className="text-center py-16 text-gray-400 bg-white rounded-xl border border-gray-100">
              <Icon name="Package" size={40} strokeWidth={1.5} className="mx-auto mb-3 opacity-30" />
              <p className="text-sm">판매중인 상담상품이 없습니다.</p>
            </div>
          ))}
        </div>
      )}

      {/* ── 상담상품 신청 탭 — 최고관리자 권한설정(상담상품신청 토글)이 켜진 경우에만 노출 ── */}
      {tab === "request" && flags.productRequest && (
          <div className="animate-fade-in">
            <div className="flex items-center gap-2 mb-2.5">
              <Icon name="Plus" size={16} strokeWidth={1.5} className="text-brand-500" />
              <h2 className="text-sm font-bold text-gray-700">상담상품 신청</h2>
            </div>
            <p className="text-xs text-gray-400 mb-3">아래 상담상품을 선택하면 브랜드 승인 후 점집에 추가됩니다.</p>

            {/* ★ 제공 방식 필터 (전체/공급가/수수료) */}
            <div className="flex gap-2 mb-3">
              {([
                { value: "ALL", label: "전체 상담상품" },
          ...
          <div className="relative bg-white w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl shadow-2xl max-h-[90vh] flex flex-col animate-slide-up sm:animate-scale-in overflow-hidden">
            ...
            <div className="flex items-center justify-between px-5 py-3.5">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-[15px] font-bold text-gray-900">상담상품 상세정보</h3>
                  <p className="text-[10px] text-gray-400">상담상품 정보와 커미션을 확인하세요</p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto min-h-0">
              <div className="px-5 pb-5 space-y-4">
                ...
                <div className="flex items-center gap-1.5 text-[10px]">
                  ...
                  <span className="text-gray-600">후기 {detailProduct.product.reviewCount}개</span>
                </div>
              </div>
            </div>
        ...
        <div className="px-4 pb-3 border-b border-gray-100 space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              style={showMyOnly ? { backgroundColor: accentColor } : {}}
            >
              <Icon name="MyPage" size={10} /> 내상담상품 ({myProducts.length})
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto min-h-0 p-3 space-y-1.5">
          {filtered.length === 0 ? (
            <div className="text-center py-12 text-gray-400">
              ...
              <p className="text-sm font-medium">{showMyOnly ? "등록한 내 상담상품이 없습니다" : "신청 가능한 상담상품이 없습니다"}</p>
                ...
                <div key={p.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-all hover:shadow-sm ${
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      {isOwn && <span className="text-[8px] bg-blue-500 text-white px-1.5 py-0.5 rounded-full font-bold flex-shrink-0">내상담상품</span>}
                    </div>
