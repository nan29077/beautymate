...
// 뷰티 전문가 대시보드: 내 라이브 코드 + 내 라이브 링크 + 공유(카카오톡/링크).
export default function SellerLiveCodeCard({ code, shopName }: { code: string; shopName: string }) {
  const [copied, setCopied] = useState<string | null>(null);

  // 공유 링크: 방송 여부에 따라 라이브 시청 페이지/외부 라이브/뷰티샵으로 서버에서 분기 리다이렉트.
  ...
  const message = `[${shopName}] 라이브 코드: ${code}\n아래 링크를 열면 방송 중일 때 라이브로, 방송 전이면 뷰티샵으로 바로 이동해요!\n${liveEntryUrl}`;
  ...
  return (
    <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl p-4 text-white">
      ...
      <p className="text-[10.5px] text-white/60 mt-2 leading-relaxed">
        고객이 이 코드로 공유 링크를 열면 <b className="text-white/80">방송 중일 땐 라이브</b>로, 방송 전이면 <b className="text-white/80">{shopName} 뷰티샵</b>으로 바로 이동해요.
      </p>
