...
import { Plus, Loader2, ImagePlus, Star, Upload, X } from "lucide-react";
...
export default function PackageRegisterForm({ mode, redirectPath }: Props) {
  ...
  const handleSubmit = async () => {
    if (!form.name.trim()) {
      await appAlert("패키지 상담상품명을 입력하세요.");
      return;
    }
    ...
    if (selectedItems.length < 2) {
      await appAlert("구성 상담상품을 최소 2개 이상 선택하세요.");
      setStep("items");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        await appAlert(err.error || "패키지 상담상품 등록에 실패했습니다.");
        return;
      }

      await appAlert("패키지 상담상품이 등록됐습니다.\n(최고관리자 승인 후 판매 가능)");
      setShowForm(false);
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-bold text-gray-900">패키지 상담상품 등록</h2>
          </div>
          <button
            type="button"
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">
                  패키지 상담상품명 <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  value={form.name}
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="구성 상담상품 합계와 무관하게 설정 가능"
                    min={0}
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="패키지 상담상품에 대한 설명을 입력하세요."
                  rows={4}
        ...
        {step === "items" && (
          <div className="px-6 py-4 border-t border-gray-100 flex items-center gap-3">
            ...
            >
              ...
              {loading ? "등록 중..." : `패키지 상담상품 등록 (구성 ${selectedItems.length}개)`}
            </button>
          </div>
        )}
      </div>
