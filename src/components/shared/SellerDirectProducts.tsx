...
import { Plus, X, Loader2, Star, Pencil, Trash2, Building2, Sparkles } from "lucide-react";
...
export default function SellerDirectProducts() {
  ...
  const handleSave = async () => {
    if (!form.name.trim()) { await appAlert("상담상품명을 입력해주세요."); return; }
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <Building2 size={15} strokeWidth={1.5} className="text-yellow-600" />
            <span className="text-sm font-bold text-gray-800">일반상담상품 탭 노출 활성화</span>
          </div>
          {loadingTab ? (
        ...
        <div className="text-[12px] text-gray-600 space-y-0.5 leading-relaxed">
          <p className="flex items-center gap-1"><Sparkles size={14} strokeWidth={1.5} className="inline-block shrink-0 text-brand-500" aria-hidden="true" /> <span><strong>빠른 상담상품 등록이란?</strong> 방송 중 빠르게 상담상품을 올려야 할 때 사용하세요.</span></p>
          <p className="flex items-center gap-1"><Sparkles size={14} strokeWidth={1.5} className="inline-block shrink-0 text-brand-500" aria-hidden="true" /> <span>스위치를 켜면 점집에 <strong>일반상담상품 탭</strong>이 생기고, 고객이 탭을 통해 상담상품을 볼 수 있습니다.</span></p>
          <p className="flex items-center gap-1"><Sparkles size={14} strokeWidth={1.5} className="inline-block shrink-0 text-brand-500" aria-hidden="true" /> <span>방송 중이 아닐 때도 상담상품을 판매하고 싶다면 등록 후 <strong>노출하기</strong>를 눌러주세요.</span></p>
          <p className="flex items-center gap-1"><Sparkles size={14} strokeWidth={1.5} className="inline-block shrink-0 text-brand-500" aria-hidden="true" /> <span>점집관리 &gt; 일반상담상품 탭의 노출 스위치와 자동으로 동기화됩니다.</span></p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Star size={16} strokeWidth={1.5} className="text-brand-500" />
          <h2 className="text-sm font-bold text-gray-700">일반상담상품 <span className="text-brand-500 font-semibold">(빠른상담상품 등록)</span></h2>
          <span className="text-[10px] bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full font-medium">{products.length}개</span>
        </div>
        <div className="flex items-center gap-2">
          {/* 일반상담상품 엑셀 대량 등록 (일반상담상품 전용 양식) */}
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-400 bg-white rounded-xl border border-gray-100">
          <Star size={40} strokeWidth={1.5} className="mx-auto mb-3 opacity-30" />
          <p className="text-sm">등록된 일반상담상품이 없습니다.</p>
          <p className="text-xs mt-1">직접 상담상품을 등록해 점집에 노출해보세요.</p>
          <button
            onClick={openCreate}
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-start justify-center p-0 sm:p-4 overflow-y-auto">
          <div className="bg-white w-full sm:max-w-md sm:rounded-2xl sm:shadow-2xl sm:my-6 min-h-screen sm:min-h-0 flex flex-col max-h-screen sm:max-h-[90vh]">
            ...
            <div className="flex-shrink-0 px-4 sm:px-6 pt-5 pb-3 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-lg font-bold text-gray-900">{editing ? "일반상담상품 수정" : "일반상담상품 등록(빠른상담상품 등록)"}</h3>
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain px-4 sm:px-6 py-5 space-y-5">
              ...
              <div>
                <label className="text-xs font-semibold text-gray-700 mb-1.5 block">상담상품 이미지 <span className="text-gray-400 font-normal">(최대 5장)</span></label>
