...
export default function PackageBuySection({
  ...
  const handleOrder = async () => {
    ...
    if (!form.buyerName || !form.buyerPhone || !form.buyerAddress) {
      await appAlert("상담 방식 정보를 모두 입력해주세요.");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        await appAlert(err.error || "예약에 실패했습니다.");
        return;
      }
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-end">
          <div className="bg-white w-full max-w-[480px] mx-auto rounded-t-2xl px-5 py-5 shadow-xl">
            <h3 className="text-base font-bold text-gray-900 mb-4">상담 방식 정보 입력</h3>
            <div className="space-y-3">
              ...
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">상담 방식 주소</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">상담 방식 메모 (선택)</label>
                <input
                  type="text"
