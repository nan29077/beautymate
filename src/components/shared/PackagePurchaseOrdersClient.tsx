...
import { Star, ChevronDown, CheckCircle2, Clock, AlertCircle } from "lucide-react";
...
const STATUS_MAP: Record<string, { label: string; color: string; icon: any }> = {
  ...
  SHIPPED: { label: "상담 방식 중", color: "text-indigo-600 bg-indigo-50", icon: Clock },
              ...
              <div key={po.id} className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="px-4 py-3 border-b border-gray-50">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <Star size={14} className="text-brand-600 flex-shrink-0" />
                ...
                {po.packageOrderItem.buyerName && (
                  <div className="px-4 py-2 bg-gray-50 text-xs text-gray-600">
                    <span className="font-medium">상담 방식:</span>{" "}
