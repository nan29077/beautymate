...
import { Disc3, Network, Ticket, Play, Monitor, Hash, Zap, HelpCircle, BarChart3, Target, Star, ListOrdered, Trophy, X } from 'lucide-react';
import {
        ...
        {!isItem && (
          <>
            ...
            <p className="text-[11px] text-gray-400 mt-3 leading-relaxed">
              ...
              {isGoal && <><b>시작</b> 이후 결제 완료 예약 수가 목표 게이지에 실시간 반영됩니다. </>}
            ...
            {noViewerJoin ? (
              <div className="flex items-start gap-2 rounded-xl bg-gray-50 border border-gray-100 px-3.5 py-3 mb-2">
                ...
                <p className="text-[12px] text-gray-500 leading-relaxed">
                  이 게임은 시청자 직접 참여가 필요 없는 게임입니다. 상담사가 직접 진행하며 결과를 발표합니다.
                </p>
              </div>
            ) : (
...
function ResultView({ type, result }: { type: string; result: Record<string, any> }) {
  ...
  if (type === "BOX_OPEN" && result.box) {
    const kindLabel: Record<string, string> = { PRODUCT: "상담상품", COUPON: "쿠폰", MISS: "꽝" };
    return (
