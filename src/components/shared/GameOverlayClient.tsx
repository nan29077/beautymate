...
import { Trophy, Target, Vote as VoteIcon, Ticket, ListOrdered, Medal, PartyPopper, Wind, Hexagon, Disc3, Network, Hash, HelpCircle, BarChart3, Gamepad2, Zap, Star, Gift } from 'lucide-react';
...
const HONEY = ["#f5a700","#ffc31a","#d99000","#ffd84d","#b37100","#ffe888"];
    ...
    return (
      ...
      >
        ...
        <div className="bg-white/90 rounded-2xl shadow-lg px-4 py-3 text-center w-full max-w-sm flex-shrink-0">
          ...
          <p className="text-[15px] font-extrabold tracking-tight text-[#2d1b69] text-center mb-1">사주메이트 GAMES</p>
    ...
    return (
      <div style={{
        ...
        <div style={{ paddingTop:14, paddingBottom:8, flexShrink:0 }}>
          ...
          <div style={{
            background:"linear-gradient(135deg,#f5a700 0%,#ffd84d 100%)",
    ...
    >
      <div className="max-w-sm mx-auto px-4 py-8 flex flex-col gap-4">
        ...
        <header className="bg-white rounded-2xl shadow-sm p-5 text-center">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <p className="text-[16px] font-extrabold tracking-tight text-[#2d1b69] text-center mb-2">사주메이트 GAMES</p>
        ...
        {/* 하단 푸터 */}
        <p className="text-center text-xs text-gray-400 pb-4">Powered by 사주메이트 라이브 게임</p>
      </div>
    </div>
  );
}
...
const TYPE_ICON: Record<string, React.ComponentType<{ className?: string; strokeWidth?: number }>> = {
  ...
  GOAL_GAUGE: Target, BOX_OPEN: Star, SEQUENTIAL: ListOrdered,
};
...
function CountPill({ n }: { n: number }) {
  return (
    <div style={{
      ...
      border:"2px solid #f5a700", borderRadius:999, padding:"8px 20px",
    ...
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:16 }}>
      <div style={{ position:"relative", width:SIZE, height:SIZE }}>
        ...
        <div style={{
          ...
          borderRadius:"50%", background:"#fff", border:"4px solid #f5a700",
          display:"flex", alignItems:"center", justifyContent:"center",
          boxShadow:"0 4px 14px rgba(0,0,0,0.3)", zIndex:4,
        }}><Hexagon size={isOverlay ? 38 : 28} strokeWidth={2.5} color="#f5a700" /></div>
      </div>
          ...
          return (
            <div key={i} style={{
              ...
              background: allRevealed
                ...
                : "linear-gradient(160deg,#ffc31a,#f5a700)",
              border: allRevealed
                ? (isWinner ? "5px solid #f5a700" : "3px solid #ddd")
              ...
              {allRevealed ? (
                <>
                  <span style={{ marginBottom:4, display:"flex" }}>
                    {isWinner
                      ? <Trophy size={isOverlay ? 40 : 26} strokeWidth={2.5} color="#f5a700" />
          ...
          return (
            <g key={`bot${i}`} transform={`translate(${colX(i)},${botY + 28})`}>
              <rect x={-34} y={-17} width={68} height={34} rx={10}
                ...
                stroke={isRevealed ? "#f5a700" : "rgba(255,255,255,0.7)"}
                strokeWidth={2.5}
              />
              <text textAnchor="middle" dominantBaseline="middle"
      ...
      {finished ? (
        ...
        <>
          ...
          <div style={{
            ...
            background:"linear-gradient(135deg,#ffc31a,#f5a700)", padding:"14px 40px",
    ...
    <Panel>
      {finished ? (
        ...
        <>
          ...
          <div style={{
            fontSize:34, fontWeight:900, color:"#f5a700",
            border:"3px dashed #f5a700", borderRadius:16, padding:"10px 34px",
          ...
          return (
            <div key={i}>
              <div style={{ display:"flex", justifyContent:"space-between", fontSize:16, fontWeight:700, color:"#5a4a2a", marginBottom:4 }}>
                <span style={{ display:"inline-flex", alignItems:"center", gap:4 }}>{win && <Trophy size={15} strokeWidth={2.5} color="#f5a700" />}{c}</span>
                <span style={{ color:"#b37100" }}>{counts[i]}표 · {pct}%</span>
              </div>
              <div style={{ height:22, borderRadius:999, background:"#f0e6cc", overflow:"hidden" }}>
                <div style={{
                  height:"100%", width:`${pct}%`, borderRadius:999,
                  background: win ? "linear-gradient(90deg,#34d399,#059669)" : "linear-gradient(90deg,#ffd84d,#f5a700)",
...
function GoalGauge({ game }: { game: GameState }) {
  ...
  return (
    <Panel>
      ...
      <div style={{ fontSize:38, fontWeight:900, color:"#f5a700" }}>
      ...
      <div style={{ width:440, maxWidth:"100%", height:34, borderRadius:999, background:"#f0e6cc", overflow:"hidden", boxShadow:"inset 0 2px 6px rgba(0,0,0,0.12)" }}>
        <div style={{
          height:"100%", width:`${pct}%`, borderRadius:999,
          background: achieved ? "linear-gradient(90deg,#34d399,#059669)" : "linear-gradient(90deg,#ffd84d,#f5a700)",
...
function BoxOpen({ game, animKey }: { game: GameState; animKey: string }) {
  ...
  if (phase === "open" && game.result?.box) {
    ...
    const OpenIcon = KIND_ICON[box.kind] || Star;
    return (
      <Panel>
        <div style={{ display:"flex", animation:"pop-in 0.5s cubic-bezier(0.18,0.89,0.32,1.28)" }}>
          <OpenIcon size={64} strokeWidth={1.75} color={miss ? "#999" : "#f5a700"} />
        </div>
        <div style={{
          fontSize:30, fontWeight:900, color: miss ? "#999" : "#7a4c0c",
          background:"#fff", padding:"12px 36px", borderRadius:999,
          border:`4px solid ${miss ? "#ccc" : "#f5a700"}`,
          boxShadow:"0 10px 30px rgba(0,0,0,0.25)",
        }}>{box.label}</div>
...
function Sequential({ game, animKey, isOverlay }: { game: GameState; animKey: string; isOverlay?: boolean }) {
  ...
  const MEDAL_COLOR = ["#f5a700", "#b0b0b0", "#cd7f32"];
  return (
    <Panel>
      <div style={{
