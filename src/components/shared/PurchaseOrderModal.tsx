...
import { X, Loader2, User as UserIcon, Phone, MapPin, Printer, Building2, CreditCard, Star, FileText } from 'lucide-react';

// 테마 캘린더 아이콘 — lucide 컴포넌트 자리(Row icon)에 그대로 사용 가능
...
<body>
  ...
  <section class="sheet">
    ...
    <table class="meta">
      ...
      <tr><th>예약번호</th><td>${escapeHtml(po.orderNumber)}</td><th>예약상태</th><td>${STATUS_LABEL[po.status] || escapeHtml(po.status)}</td></tr>
      ${po.campaignTitle ? `<tr><th>단체 상담</th><td colspan="3">${escapeHtml(po.campaignTitle)}</td></tr>` : ""}
    </table>
    ...
    <h2>상담사 정보</h2>
    <table class="meta">
      <tr><th>점집명</th><td>${escapeHtml(po.seller.shopName) || "-"}</td><th>상담사명</th><td>${escapeHtml(po.seller.sellerName) || "-"}</td></tr>
    </table>

    <h2>발주 상담상품</h2>
    <table class="items">
      <thead><tr><th>상담상품명</th><th style="width:100px">옵션</th><th style="width:150px">공급자</th><th style="width:50px">수량</th><th style="width:80px">단가</th><th style="width:90px">합계</th></tr></thead>
      <tbody>${itemRows}</tbody>
    </table>

    <h2>결제 정보</h2>
    <table class="meta">
      <tr><th>결제방법</th><td>${escapeHtml(po.payment.method)}</td><th>상담상품합계</th><td>${won(po.payment.itemsTotal)}</td></tr>
    </table>
    <table class="totals">
      <tr><th>상담상품 합계</th><td>${won(po.payment.totalAmount)}</td></tr>
    ...
    ${po.payment.partial ? `<p class="note">※ 본 발주서에는 귀하가 공급한 상담상품만 표시되며, 결제금액은 예약 전체 기준입니다.</p>` : ""}
    ...
    <table class="meta">
      ...
      ${po.shipping.shippedAt ? `<tr><th>상담 시작</th><td colspan="3">${fmt(po.shipping.shippedAt)}</td></tr>` : ""}
    </table>
  </section>
</body></html>`;
}
        ...
        {loading ? (
          ...
          <div className="p-5 space-y-4 max-h-[72vh] overflow-y-auto">
            ...
            <div className="bg-gray-50 rounded-xl p-4 space-y-2">
              ...
              <Row icon={CreditCard} label="예약상태" value={STATUS_LABEL[po.status] || po.status} />
              {po.campaignTitle && <Row icon={Star} label="단체 상담" value={po.campaignTitle} />}
            </div>

            {/* 고객 정보 */}
            <div className="bg-gray-50 rounded-xl p-4 space-y-2">
              <p className="text-xs font-bold text-gray-700 mb-1 flex items-center gap-1">
                <Icon name="MyPage" size={12} /> 고객 정보
              </p>
            ...
            <div className="bg-gray-50 rounded-xl p-4 space-y-2">
              <p className="text-xs font-bold text-gray-700 mb-1 flex items-center gap-1">
                <Icon name="Store" size={12} /> 상담사 정보
              </p>
              <Row icon={Building2} label="점집명" value={po.seller.shopName} />
              <Row icon={UserIcon} label="상담사명" value={po.seller.sellerName} />
            </div>

            {/* 발주 상담상품 (상담상품별 공급자 포함) */}
            <div>
              <p className="text-xs font-bold text-gray-700 mb-2 flex items-center gap-1">
                <Icon name="Package" size={12} /> 발주 상담상품
              </p>
            ...
            <div className="bg-brand-50 rounded-xl p-4 space-y-1.5">
              ...
              {po.payment.partial && (
                <p className="text-[10px] text-gray-400 mt-1">
                  ※ 본 발주서에는 귀하가 공급한 상담상품만 표시되며, 결제금액은 예약 전체 기준입니다.
                </p>
              )}
            </div>
            ...
            {po.shipping.name && (
              <div className="bg-gray-50 rounded-xl p-4 space-y-2">
                <p className="text-xs font-bold text-gray-700 mb-1 flex items-center gap-1">
                  <Icon name="Truck" size={12} /> 상담 방식 정보
                </p>
                ...
                {po.shipping.shippedAt && <Row icon={BeeCalendarIcon} label="상담 시작" value={fmt(po.shipping.shippedAt)} />}
              </div>
            )}
          </div>
        ) : null}
