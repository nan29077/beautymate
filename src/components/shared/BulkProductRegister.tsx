...
import { X, Upload, Download, Loader2, FileSpreadsheet, CheckCircle2, AlertTriangle, Table2, Star, Percent } from "lucide-react";
...
export default function BulkProductRegister({ mode, brands = [], kind = "catalog", onSuccess }: Props) {
  ...
  const downloadTemplate = async () => {
    ...
    const notices = isDirect
      ...
      : isCommission
      ? [
          `📋 사주메이트 상담상품 대량 등록 양식 (수수료 제공 방식)  ·  대상 계정: ${MODE_LABEL[mode]}`,
          `⚠️ 노란색 안내 행은 모두 삭제하고, 파란색 '상담상품명' 제목 행 아래에 상담상품 정보만 입력하세요.  (예시)로 시작하는 행은 등록되지 않습니다.`,
      ...
      : [
          `📋 사주메이트 상담상품 대량 등록 양식 (공급가 제공 방식)  ·  대상 계정: ${MODE_LABEL[mode]}`,
          `⚠️ 노란색 안내 행은 모두 삭제하고, 파란색 '상담상품명' 제목 행 아래에 상담상품 정보만 입력하세요.  (예시)로 시작하는 행은 등록되지 않습니다.`,
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {showTypeStep && (
                  <span className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full ${
                    ...
                    {isCommission ? <Percent size={11} /> : <Star size={11} />}
