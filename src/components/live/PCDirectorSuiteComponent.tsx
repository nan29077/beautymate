...
import { Heart, Share2, Calendar, Send, Radio, Clock, Eye, Loader2, ChevronRight, Volume2, VolumeX, Star, Gift, Info, Tag, Bell, BookOpen, ArrowLeft, Copy, Ticket, CheckCircle, ExternalLink, Building2, Play, Pause, Youtube, Gamepad2 } from "lucide-react";
              ...
              <div className="w-[68px] h-[68px] rounded-xl bg-gray-100 border border-white/10 overflow-hidden relative">
                {lp.product.thumbnail
                  ? <img
                    ...
                    />
                  : <span className="absolute inset-0 flex items-center justify-center"><Star size={20} className="text-white/30" /></span>
                }
              </div>
              {lp.isActive && isLive && (
                ...
                <div className="w-12 h-12 rounded-xl bg-gray-100 flex-shrink-0 overflow-hidden relative">
                  {lp.product.thumbnail
                    ? <img
                        ...
                        onError={e => { e.currentTarget.src = '/no-image.png'; }}
                      />
                    : <span className="absolute inset-0 flex items-center justify-center"><Star size={16} className="text-white/30" /></span>
                  }
                </div>
                <div className="flex-1 min-w-0">
