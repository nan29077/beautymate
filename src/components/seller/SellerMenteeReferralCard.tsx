...
export default function SellerMenteeReferralCard({ referralCode: initialCode, referralLink: initialLink }: Props) {
  ...
  const share = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: "상담사 가입 추천", text: shareText, url: shareUrl });
        return;
      } catch {}
    }
    ...
    <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl p-4 text-white">
      ...
      {!code ? (
        ...
        <div className="space-y-2.5">
          {/* 추천인코드 */}
          <div>
            <p className="text-[10px] font-medium text-white/40 mb-1">상담사가입 추천인코드</p>
