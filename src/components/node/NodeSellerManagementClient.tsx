  ...
  return (
    <>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
        <div>
          <h1 className="text-xl font-bold text-gray-900">상담사 관리</h1>
          <p className="text-sm text-gray-500">전체 상담사 {sellers.length}명</p>
        </div>
        <button
          onClick={() => {
      ...
      {tempPassword && (
        <div className="bg-teal-50 border border-teal-200 rounded-xl p-4 mb-5">
          <p className="text-xs font-semibold text-teal-700 mb-2 flex items-center gap-1.5">
            ...
            상담사 계정이 생성되었습니다. 임시 비밀번호를 전달해주세요.
          </p>
          ...
          <p className="text-[10px] text-teal-500 mt-2">※ 상담사에게 임시 비밀번호를 안전하게 전달하세요. 이 창을 닫으면 다시 확인할 수 없습니다.</p>
            ...
            <div className="bg-gray-50 rounded-xl p-4 space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="sm:col-span-2">
                  <label className="block text-xs font-medium text-gray-600 mb-1">점집명</label>
                  <input
                    type="text"
                    className="input-field text-sm"
                    placeholder="미입력 시 '이름의 점집'"
                    value={form.shopName}
      ...
      <div className="space-y-3">
        {filteredSellers.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-xl border border-gray-100">
            ...
            <p className="text-sm text-gray-500">등록된 상담사가 없습니다</p>
            <p className="text-xs text-gray-400 mt-1">상담사 초대 버튼을 눌러 새로운 상담사를 추가하세요</p>
          </div>
        ) : (
