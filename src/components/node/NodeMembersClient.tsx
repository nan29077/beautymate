...
import { Building2, Crown, X, Phone, UserCog } from 'lucide-react';
...
export default function NodeMembersClient({
  ...
  const TABS: { key: TabType; label: string; icon: any; count: number }[] = [
    ...
    { key: "sellers", label: "상담사", icon: Building2, count: sellers.length },
  ];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-sm text-gray-500">
            중간관리자 {middleAdmins.length} · 브랜드 {brands.length} · 상담사 {sellers.length}
          </p>
        </div>
        {tab === "middleAdmins" && (
          <button
              ...
              <div key={m.id} className="bg-white rounded-xl border border-gray-100 overflow-hidden">
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-4 mt-3 pt-3 border-t border-gray-50 flex-wrap text-xs text-gray-400">
                    ...
                    <span className="flex items-center gap-1"><Icon name="Store" size={11} />{m.sellerCount}명 상담사</span>
          ...
          {filteredBrands.length === 0 ? (
            ...
            brandItems.map((b) => (
              <div key={b.id} className="bg-white rounded-xl border border-gray-100 p-4">
                ...
                <div className="flex items-center gap-4 mt-3 pt-3 border-t border-gray-50 flex-wrap text-xs text-gray-400">
                  <span className="flex items-center gap-1"><Icon name="Package" size={11} />{b.productCount}개 상담상품</span>
        ...
        <div className="space-y-3">
          {filteredSellers.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-xl border border-gray-100">
              ...
              <p className="text-sm text-gray-500">담당 상담사가 없습니다</p>
            </div>
          ) : (
            sellerItems.map((s) => (
