  ...
  async function submit(p: NodeProduct, finalize: boolean) {
    ...
    if (finalize) {
      ...
      const ok = await appConfirm(
        `"${p.name}" 상담상품을 마진 ${draft.type === "RATE" ? `${marginNum}%` : won(marginNum)} 으로 최종 등록할까요?\n최종 등록 후 최고관리자 승인 단계로 넘어갑니다.`,
      );
      if (!ok) return;
    }
      ...
      <section>
        ...
        {pending.length === 0 ? (
          <div className="bg-white rounded-xl border border-gray-100 p-8 text-center">
            ...
            <p className="text-sm text-gray-400">노드로 들어온 대기 상담상품이 없습니다.</p>
          </div>
        ) : (
      ...
      <section>
        ...
        {approved.length === 0 ? (
          <div className="bg-white rounded-xl border border-gray-100 p-6 text-center">
            <p className="text-sm text-gray-400">최종 등록한 상담상품이 없습니다.</p>
          </div>
        ) : (
